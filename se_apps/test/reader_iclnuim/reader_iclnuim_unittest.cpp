    ...
    virtual void SetUp()
    {
        ...
        for (size_t frame = 0; frame < num_frames_; ++frame) {
            ...
            for (size_t pixel = 0; pixel < width_ * height_; ++pixel) {
                fs << std::setprecision(10) << static_cast<float>(frame + pixel) << " ";
            }
            fs.close();

            // Generate a 16-bit RGB PNG image.
            std::unique_ptr<uint16_t[]> rgb_data(new uint16_t[3 * width_ * height_]);
            for (size_t pixel = 0; pixel < width_ * height_; ++pixel) {
        ...
        config_ = {0, false, true, scene_dirname_, gt_filename_};
    }
