...
enum PlottingHint {
    ...
    ,
    phForceRepaint =
        0x002 ///< <tt>0x002</tt> causes an immediate repaint() instead of a soft update() when QCustomPlot::replot() is called with parameter \ref QCustomPlot::rpHint.
...
class QCPAxisPainterPrivate {
    ...
    virtual void
    drawTickLabel(QCPPainter* painter, double x, double y, const TickLabelData& labelData) const;
    virtual TickLabelData getTickLabelData(const QFont& font, const QString& text) const;
    virtual QPointF getTickLabelDrawOffset(const TickLabelData& labelData) const;
    virtual void
    getMaxTickLabelSize(const QFont& font, const QString& text, QSize* tickLabelsSize) const;
};
...
class QCP_LIB_DECL QCPAbstractPlottable : public QCPLayerable {
    ...
    virtual double
    selectTest(const QPointF& pos, bool onlySelectable, QVariant* details = 0) const = 0;
...
class QCP_LIB_DECL QCustomPlot : public QWidget {
    ...
    bool
    addLayer(const QString& name, QCPLayer* otherLayer = 0, LayerInsertMode insertMode = limAbove);
    ...
    QCPLayerable*
    layerableAt(const QPointF& pos, bool onlySelectable, QVariant* selectionDetails = 0) const;
...
class QCP_LIB_DECL QCPColorScale : public QCPLayoutElement {
    ...
    Q_SLOT
    void setDataScaleType(QCPAxis::ScaleType scaleType);
    Q_SLOT
...
class QCP_LIB_DECL QCPCurve : public QCPAbstractPlottable {
    ...
    void
    setData(const QVector<double>& t, const QVector<double>& key, const QVector<double>& value);
    void setData(const QVector<double>& key, const QVector<double>& value);
    void setScatterStyle(const QCPScatterStyle& style);
    ...
    void addData(double key, double value);
    void
    addData(const QVector<double>& ts, const QVector<double>& keys, const QVector<double>& values);
...
class QCP_LIB_DECL QCPItemStraightLine : public QCPAbstractItem {
    ...
    // non-virtual methods:
    double
    distToStraightLine(const QVector2D& point1, const QVector2D& vec, const QVector2D& point) const;
...
class QCP_LIB_DECL QCPItemLine : public QCPAbstractItem {
    ...
    // non-virtual methods:
    QLineF
    getRectClippedLine(const QVector2D& start, const QVector2D& end, const QRect& rect) const;
    QPen mainPen() const;
};
...
class QCP_LIB_DECL QCPItemText : public QCPAbstractItem {
    ...
    // non-virtual methods:
    QPointF
    getTextDrawPoint(const QPointF& pos, const QRectF& rect, Qt::Alignment positionAlignment) const;
    QFont mainFont() const;
...
class QCP_LIB_DECL QCPItemPixmap : public QCPAbstractItem {
    ...
    // non-virtual methods:
    void
    updateScaledPixmap(QRect finalRect = QRect(), bool flipHorz = false, bool flipVert = false);
