...
enum CameraState { CAMERA_RUNNING, CAMERA_CLOSED, CAMERA_PAUSED, CAMERA_LIVE, UNINITIALISED };
...
class Viewer : public QWidget {
    ...
    private:
    enum MixType { ALLOW, IGNORE, WARNING };
    FImage image;
    OpenGLWidget* opengl;
...
class Viewers : public QWidget { //QMainWindow{
    ...
    void setBufferSize(int width, int height);
    void addViewer(FImage image, const char* title, const char* help, bool* active);
    void
    addViewer(FImage image, const char* title, const char* help, bool* active, bool allowScale);
    void addViewer(PerfStats* stats, const char* title, const char* help, bool* active);
    void setCurrentViewer(Viewer* viewer);
    void pauseStream(int id);
    void resumeStream(int id);
    void setZoom(float zoomX, float zoomY);
    void setZoom(float zoom);
    void addDefConfig(int viewer, QString name, bool enabled, QStringList visible);
    void setStatEntry(QString title, std::vector<const char*> entries, bool enabled);
