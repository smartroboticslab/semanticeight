...
void ApplicationWindow::writeConfig(QString filename)
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
        return;
    file.write("#This file is used to configure the visual appearance and behaviour of kfusion");
...
void ApplicationWindow::readConfig(QString filename)
{
    ...
    while (!file.atEnd()) {
        ...
        QString line(tmp);
        QStringList strings =
            line.split("\""); //divide by quote, should seperate out quoted strings
        QStringList words = strings.at(0).split(QRegExp("[,\\s]+"));
        strings.pop_front();
        if (words.at(0).toLower() == "data_dir") {
            dirname = strings.at(0);
            if (debug)
                std::cerr << "Default data dir for ONI files: " << dirname.toStdString()
                          << std::endl;
        }
        if (words.at(0).toLower() == "viewer") {
            ...
            if (debug)
                std::cerr << "Viewer " << viewerID << " set to " << title.toStdString() << " and "
        ...
        if (words.at(0).toLower() == "enable_looping") {
            ...
            if (debug)
                std::cerr << "Sequence looping  " << (loop ? "enabled" : "disabled") << std::endl;
        }
        ...
        if (words.at(0).toLower() == "conditional_breakpoints") {
            for (int j = 0; j < strings.length(); j = j + 2) {
                if (strings.at(j) != "") {
                    ...
                    setConditionalBreakpoint(strings.at(j), words.at(2).toInt());
                }
            }
        }
    }
...
void ApplicationWindow::setFrameRateLabel(float value, int frame)
{
    char tmp[256];
    sprintf(
        tmp, "%s frame rate: %0.2f frame: %d", frameRateField.toStdString().c_str(), value, frame);
...
CameraState ApplicationWindow::setCameraState(CameraState state, std::string file = "")
{
    ...
    if (cameraCallback) {
        ...
        if ((newState == CAMERA_RUNNING) || (newState == CAMERA_LIVE)) {
            if ((newState == CAMERA_LIVE)
                || ((file == "")
                    && (liveButton->isChecked() || (lastState == CAMERA_CLOSED)
...
std::string
ApplicationWindow::fileSaveSelector(std::string title, std::string directory, std::string filter)
{
    QString filename =
        QFileDialog::getSaveFileName(this, title.c_str(), directory.c_str(), filter.c_str());
    return (filename.toStdString());
}
std::string
ApplicationWindow::fileOpenSelector(std::string title, std::string directory, std::string filter)
{
    QString filename =
        QFileDialog::getOpenFileName(this, title.c_str(), directory.c_str(), filter.c_str());
    return (filename.toStdString());
}
...
void ApplicationWindow::fileMenuSlot(QAction* action)
{
    ...
    case OPEN_SCENE: {
        ...
        QString filename = QFileDialog::getExistingDirectory(
            this, "Select scene directory...", dirname, QFileDialog::ShowDirsOnly);
        if (filename != "") {
            setCameraState(CAMERA_CLOSED, "");
            dirname = (QFileInfo(filename)).path();
            setCameraState(CAMERA_RUNNING, filename.toStdString());
...
void ApplicationWindow::setDumpFunction(const char* label, void (*_callback)())
{
    for (int i = 0; i < fileMenu->actions().length(); i++) {
        if ((fileMenu->actions().at(i)->isSeparator())
            && fileMenu->actions().at(i)->text() == "Dump") {
            QAction* action = new QAction(label, this);
            QAction* tmp = fileMenu->actions().at(i + 1 + dumpFunctions.length());
...
void ApplicationWindow::callDumpFunction(QAction*)
{
}
...
void ApplicationWindow::setCameraViewFunction(void (*_callback)(bool), bool initial)
{
...
void ApplicationWindow::startLive()
{
    ...
    if (isChecked) {
        ...
        setCameraState(CAMERA_CLOSED, "");
        ((QToolButton*) QObject::sender())->setIcon(QIcon(":/icons/images/film.png"));
    }
}
                                          ...
                                          CameraState (*_callback)(CameraState, std::string))
{
    ...
    if (cameraButton == NULL) {
        ...
        liveButton->setToolTip("Toggle between live camera and recorded sequence");
        liveButton->setIcon(QIcon(":/icons/images/film.png"));
        ...
        restartButton->setToolTip("Return to start of recorded sequence, and reset model");
        ...
        connect(cameraButton, SIGNAL(clicked()), this, SLOT(cameraButtonPress()));
...
void ApplicationWindow::setRotPointer(Eigen::Matrix4f* rotVar)
{
    ...
    leftButton->setToolTip("Rotate model anticlockwise around y axis, after enabling fixed view ");
    ...
    rightButton->setToolTip("Rotate model clockwise around y axis, after enabling fixed view ");
    ...
    upButton->setToolTip("Rotate model anticlockwise around x axis, after enabling fixed view ");
    ...
    downButton->setToolTip("Rotate model clockwise around x axis, after enabling fixed view ");
...
void ApplicationWindow::changeCheckBox()
{
    for (int i = 0; i < checkboxes.length(); i++) {
        if (checkboxes.at(i)->checkbox == QObject::sender()) {
            *(checkboxes.at(i)->value) = ((QCheckBox*) QObject::sender())->isChecked();
            break;
        }
    }
}

ButtonChoices::ButtonChoices(QWidget* parent,
                             const char* name,
                             void* variable,
                             VarType type,
                             void (*callback)())
...
void ButtonChoices::updateValue()
{
    ...
    for (int i = 0; i < (_button->menu()->actions()).length(); i++) {
        if (type == INT) {
            if (*((int*) value) == _button->menu()->actions().at(i)->data().toInt()) {
        ...
        if (type == FLOAT) {
            if (*((float*) value) == _button->menu()->actions().at(i)->data().toFloat()) {
                _button->menu()->actions().at(i)->setChecked(true);
                lab += _button->menu()->actions().at(i)->text();
                _button->setText(lab);
                return;
    ...
    char first[256];
    char* tmp = first;
    strcpy(first, (char*) ((_button->menu()->actions().at(0)->text().toStdString()).c_str()));
    QString newEntry = QString::number(type == INT ? *((int*) value) : *((float*) value));
...
void ApplicationWindow::addButtonChoices(const char* label,
                                         ...
                                         void (*callback)())
{
    ButtonChoices* container = new ButtonChoices(this, label, variable, INT, callback);
                                         ...
                                         void (*callback)())
{
    ...
    ButtonChoices* container = new ButtonChoices(this, label, variable, FLOAT, callback);
...
void ButtonChoices::changeButtonChoices(QAction* action)
{
    ...
    sprintf(tmp, "%s: %s", name->toStdString().c_str(), action->text().toStdString().c_str());
    _button->setText(tmp);
    if (callback)
        callback();
}

void ApplicationWindow::addSlider(const char* label, int min, int max, int* value)
{
...
void ApplicationWindow::addDial(const char* label, int min, int max, int* value)
{
...
void ApplicationWindow::timerEvent(QTimerEvent*)
{
    ...
    if ((*cameraOpen || requireUpdate) && !viewers->holdProcessing() && !holdProcessing) {
        if (callback != NULL) {
            (callback)();
        }
    }
...
void ApplicationWindow::addConditionalBreakpoint(QString label, BreakpointType type, int value)
{
...
void ApplicationWindow::addConditionalBreakpoint(QString label, BreakpointType type, double value)
{
    ConditionalBreakpoint* bp = new (ConditionalBreakpoint);
    bp->name = label;
    bp->type = type;
...
ConditionalBreakpoint* ApplicationWindow::getConditionalBreakpoint(QString label)
{
    for (int i = 0; i < conditionalBreakpoints.count(); i++) {
        if (conditionalBreakpoints.at(i)->name == label)
            std::cerr << label.toStdString() << " " << *((int*) conditionalBreakpoints.at(i)->value)
                      << std::endl;
...
void ApplicationWindow::update(int fid, CameraState cameraState)
{
                ...
                if (stats != nullptr) {
                    ...
                    if (value == *((int*) (conditionalBreakpoints.at(i)->value))) {
                        ...
                        messageBox = new QMessageBox(
                            QMessageBox::Information, "Breakpoint", message, QMessageBox::Ok);
                    }
                }
            }
        }
    for (int i = 0; i < breakpoints.length(); i++) {
        if (breakpoints.at(i) == fid) {
            ...
            messageBox =
                new QMessageBox(QMessageBox::Information, "Breakpoint", message, QMessageBox::Ok);
        }
    }
...
void DebugWindow::addLineEdit(const char* label, const char* value, const char* tooltip)
{
...
void ApplicationWindow::showDebugWindow()
{
    ...
    form->addLineEdit(
        "frame", values.toStdString().c_str(), "Space seperated list of frames to break at");
    ...
    for (int i = 0; i < conditionalBreakpoints.length(); i++) {
        if ((conditionalBreakpoints.at(i))->type == COND_BOOL) {
            form->addBoolTriRadioBox((conditionalBreakpoints.at(i))->name.toStdString().c_str(),
                                     *((int*) conditionalBreakpoints.at(i)->value));
        }
    }
