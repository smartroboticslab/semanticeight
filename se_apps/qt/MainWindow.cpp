...
QString Viewer::getOptions()
{
    ...
    for (int i = 0; i < optionButton->menu()->actions().length(); i++) {
        ...
        ret = ret + (optionButton->menu()->actions().at(i)->isChecked() ? "1" : "0");
    }
    return (ret);
}
...
void Viewer::setStreamImage(Stream* stream)
{
    ...
    if (stream->active != NULL) {
        ...
        checkbox->setChecked(true);
        checkbox->setStyleSheet("QLabel { background-color : white; color : blue; }");
    }
    else {
                    ...
                    if (allowMixedPlot == WARNING) {
                        *holdProcessing = true;
                        ...
                        case QMessageBox::No:
                            for (int j = 0; j < i; j++) {
                                allowMixedPlot = IGNORE;
                                QAction* action = optionButton->menu()->actions().at(j);
                                if (action->isSeparator()) {
                                    section++;
                                    continue;
                                }
...
void Viewer::setOptionMenu()
{
    if (_stream->statistics != NULL) {
        ...
        int i = 0;
        int colorRIndex[16] = {0, 255, 0, 0, 255, 255, 0, 128, 0, 0, 128, 128, 0, 128, 255, 64};
        int colorGIndex[16] = {0, 0, 255, 0, 255, 0, 255, 0, 128, 0, 128, 0, 0, 64, 128, 255};
        int colorBIndex[16] = {0, 0, 0, 255, 0, 255, 255, 0, 0, 128, 0, 128, 128, 255, 64, 128};

        PerfStats::Type lastType = PerfStats::UNDEFINED;
        for (std::map<int, std::string>::const_iterator kt = stats->order_.begin();
             kt != stats->order_.end();
             kt++) {
            ...
            connect(action, SIGNAL(changed()), this, SLOT(plotWaveSelectionChanged()));
            ...
            if (optionButton->menu() != NULL) {
                ...
                for (int i = 0; i < optionButton->menu()->actions().length(); i++) {
                    ...
                    if (optionButton->menu()->actions().at(i)->isChecked()) {
                        ...
                        QString name = optionButton->menu()->actions().at(i)->text();

                        double value =
                            _stream->statistics->getLastDataMerged(name.toStdString().c_str());

                        if (_stream->statistics->getType(name.toStdString().c_str())
                            == PerfStats::FRAME) {
                        ...
                        if (plot->yAxis->label() == "") {
                            switch (_stream->statistics->getType(name.toStdString().c_str())) {
...
void Viewers::addViewer(PerfStats* stats, const char* title, const char* help, bool* active)
{
...
void Viewers::addDefConfig(int viewerID, QString name, bool enabled, QStringList visible)
{
    if (viewerID < viewers.length()) {
        ...
        for (int i = 0; i < streams.length(); i++) {
            if (QString(streams.at(i)->title) == name) {
                Viewer* viewer = new Viewer(streams.at(i), viewers.length(), this);
...
void Viewers::setStatEntry(QString title, std::vector<const char*> entries, bool enabled)
{
    QStringList visible;
...
void Viewers::addViewer(Stream* stream)
{
    ...
    if (stream->statistics == NULL) {
        ...
        viewer->setHoldVariable(&_holdProcessing);
        viewers.append(viewer);
        layout->addWidget(viewer, layout->rowCount() > 2 ? layout->rowCount() : 2, 0, 1, 4);
    }
}
void Viewers::addViewer(FImage image, const char* title, const char* help, bool* active)
{
    addViewer(image, title, help, active, false);
}

void Viewers::addViewer(FImage image,
                        const char* title,
                        const char* help,
                        bool* active,
                        bool allowScale)
