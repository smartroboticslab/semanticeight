...
void QCPLayoutElement::setOuterRect(const QRect& rect)
{
    if (mOuterRect != rect) {
        mOuterRect = rect;
        mRect = mOuterRect.adjusted(
            mMargins.left(), mMargins.top(), -mMargins.right(), -mMargins.bottom());
    }
}
            ...
            foreach (QCP::MarginSide side,
                        ...
                        side)) // this side's margin shall be calculated automatically
                {
                    ...
                    if (QCP::getMarginValue(newMargins, side)
                        ...
                        QCP::setMarginValue(
                            newMargins, side, QCP::getMarginValue(mMinimumMargins, side));
                }
            }
 ...
 */
double
QCPLayoutElement::selectTest(const QPointF& pos, bool onlySelectable, QVariant* details) const
{
    Q_UNUSED(details)
...
QCPGrid::QCPGrid(QCPAxis* parentAxis) :
        QCPLayerable(parentAxis->parentPlot(), "", parentAxis), mParentAxis(parentAxis)
{
            ...
            for (int i = lowTick; i <= highTick; ++i) {
                if (qAbs(mParentAxis->mTickVector.at(i)) < epsilon) {
                    ...
                    painter->drawLine(QLineF(
                        t, mParentAxis->mAxisRect->bottom(), t, mParentAxis->mAxisRect->top()));
                    break;
                }
            }
        }
        ...
        if (mZeroLinePen.style() != Qt::NoPen && mParentAxis->mRange.lower < 0
            ...
            for (int i = lowTick; i <= highTick; ++i) {
                if (qAbs(mParentAxis->mTickVector.at(i)) < epsilon) {
                    ...
                    painter->drawLine(QLineF(
                        mParentAxis->mAxisRect->left(), t, mParentAxis->mAxisRect->right(), t));
                    break;
                }
            }
        }
...
void QCPAxis::setupTickVectors()
{
    ...
    if (mSubTickCount > 0) {
        ...
        int highTick = mHighestVisibleTick < mTickVector.size() - 1 ? mHighestVisibleTick + 1
                                                                    : mHighestVisibleTick;
        for (int i = lowTick + 1; i <= highTick; ++i) {
            subTickStep =
                (mTickVector.at(i) - mTickVector.at(i - 1)) / (double) (mSubTickCount + 1);
        ...
        if (mTickLabelType == ltNumber) {
            for (int i = mLowestVisibleTick; i <= mHighestVisibleTick; ++i)
                mTickVectorLabels[i] = mParentPlot->locale().toString(
                    mTickVector.at(i), mNumberFormatChar, mNumberPrecision);
        }
    ...
    4,                             \
    7,                             \
    0) // use fromMSecsSinceEpoch function if available, to gain sub-second accuracy on tick labels (e.g. for format "hh:mm:ss:zzz")
                mTickVectorLabels[i] = mParentPlot->locale().toString(
...
void QCPAxis::generateAutoTicks()
{
    ...
    else // mScaleType == stLogarithmic
    {
        ...
        {
            mTickVector.clear();
            qDebug() << Q_FUNC_INFO << "Invalid range for logarithmic plot: " << mRange.lower << "-"
                     << mRange.upper;
        }
    }
}
...
void QCPAxisPainterPrivate::draw(QCPPainter* painter)
{
    ...
    if (QCPAxis::orientation(type) == Qt::Horizontal)
        baseLine.setPoints(origin + QPointF(xCor, yCor),
                           origin + QPointF(alignmentRect.width() + xCor, yCor));
    else
        baseLine.setPoints(origin + QPointF(xCor, yCor),
                           origin + QPointF(xCor, -alignmentRect.height() + yCor));
    if (reversedEndings)
        baseLine = QLineF(
            baseLine.p2(),
    ...
    int selTickLabelSize = (QCPAxis::orientation(type) == Qt::Horizontal ? tickLabelsSize.height()
    ...
    if (type == QCPAxis::atLeft) {
        ...
        mAxisSelectionBox.setCoords(alignmentRect.left(),
                                    origin.y() - selAxisOutSize,
                                    alignmentRect.right(),
                                    origin.y() + selAxisInSize);
        mTickLabelsSelectionBox.setCoords(alignmentRect.left(),
                                          origin.y() - selTickLabelOffset - selTickLabelSize,
                                          alignmentRect.right(),
                                          origin.y() - selTickLabelOffset);
        mLabelSelectionBox.setCoords(alignmentRect.left(),
...
QCPAxisPainterPrivate::getTickLabelData(const QFont& font, const QString& text) const
{
    ...
    if (result.baseFont.pointSizeF()
        > 0) // On some rare systems, this sometimes is initialized with -1 (Qt bug?), so we check here before possibly setting a negative value in the next line
    ...
    if (useBeautifulPowers) {
        ...
        if (abbreviateDecimalPowers && result.basePart == "1")
            ...
            result.basePart +=
                (numberMultiplyCross ? QString(QChar(215)) : QString(QChar(183))) + "10";
        result.expPart = text.mid(ePos + 1);
        // clip "+" and leading zeros off expPart:
        while (result.expPart.length() > 2
               && result.expPart.at(1)
                   == '0') // length > 2 so we leave one zero when numberFormatChar is 'e'
        ...
        result.expBounds =
            QFontMetrics(result.expFont).boundingRect(0, 0, 0, 0, Qt::TextDontClip, result.expPart);
 ...
 */
double
QCPAbstractItem::distSqrToLine(const QPointF& start, const QPointF& end, const QPointF& point) const
{
    QVector2D a(start);
    QVector2D b(end);
    QVector2D p(point);
 ...
 */
double
QCPAbstractItem::rectSelectTest(const QRectF& rect, const QPointF& pos, bool filledRect) const
{
    double result = -1;
...
void QCustomPlot::replot(QCustomPlot::RefreshPriority refreshPriority)
{
    ...
    mPaintBuffer.fill(mBackgroundBrush.style() == Qt::SolidPattern ? mBackgroundBrush.color()
                                                                   : Qt::transparent);
    QCPPainter painter;
    ...
    if (painter.isActive()) {
        painter.setRenderHint(
            QPainter::
                HighQualityAntialiasing); // to make Antialiasing look good if using the OpenGL graphicssystem
...
void QCustomPlot::resizeEvent(QResizeEvent* event)
{
    ...
    setViewport(rect());
    replot(
        rpQueued); // queued update is important here, to prevent painting issues in some contexts
}
...
void QCustomPlot::mousePressEvent(QMouseEvent* event)
{
    ...
    mMousePressPos =
        event
            ->pos(); // need this to determine in releaseEvent whether it was a click (no position change between press and release)
 ...
 */
QCPLayerable*
QCustomPlot::layerableAt(const QPointF& pos, bool onlySelectable, QVariant* selectionDetails) const
{
...
QPixmap QCustomPlot::toPixmap(int width, int height, double scale)
{
    ...
    result.fill(
        mBackgroundBrush.style() == Qt::SolidPattern
            ...
            : Qt::
                transparent); // if using non-solid pattern, make transparent now and draw brush pattern later
    QCPPainter painter;
        ...
        if (mPeriodic) {
            for (int i = 0; i < n; ++i) {
                int index = (int) (qLn(data[dataIndexFactor * i] / range.lower)
                                   / qLn(range.upper / range.lower) * mLevelCount)
                    % mLevelCount;
                if (index < 0)
                    index += mLevelCount;
                scanLine[i] = mColorBuffer.at(index);
    ...
    if (mColorStops.size() > 1) {
        ...
        for (int i = 0; i < mLevelCount; ++i) {
                       ...
                       .constEnd()) // position is on or after last stop, use color of last stop
            {
                ...
                it
                == mColorStops
                       .constBegin()) // position is on or before first stop, use color of first stop
            {
...
void QCPAxisRect::drawBackground(QCPPainter* painter)
{
    ...
    if (!mBackgroundPixmap.isNull()) {
        if (mBackgroundScaled) {
            ...
            painter->drawPixmap(mRect.topLeft(),
                                mScaledBackgroundPixmap,
            ...
            painter->drawPixmap(
                mRect.topLeft(), mBackgroundPixmap, QRect(0, 0, mRect.width(), mRect.height()));
        }
    }
}
            ...
            if (mRangeZoom.testFlag(Qt::Horizontal)) {
                ...
                if (mRangeZoomHorzAxis.data())
                    mRangeZoomHorzAxis.data()->scaleRange(
                        factor, mRangeZoomHorzAxis.data()->pixelToCoord(event->pos().x()));
            }
            if (mRangeZoom.testFlag(Qt::Vertical)) {
                factor = pow(mRangeZoomFactorVert, wheelSteps);
                if (mRangeZoomVertAxis.data())
                    mRangeZoomVertAxis.data()->scaleRange(
                        factor, mRangeZoomVertAxis.data()->pixelToCoord(event->pos().y()));
            }
            mParentPlot->replot();
        }
    }
...
/* inherits documentation from base class */
double
QCPAbstractLegendItem::selectTest(const QPointF& pos, bool onlySelectable, QVariant* details) const
{
    Q_UNUSED(details)
    if (!mParentPlot)
        return -1;
...
QSize QCPPlottableLegendItem::minimumSizeHint() const
{
    ...
    textRect =
        fontMetrics.boundingRect(0, 0, 0, iconSize.height(), Qt::TextDontClip, mPlottable->name());
...
{
    if (!hasItem(item)) {
        return addElement(0, columnCount(), item); //Flipped
    }
    else
        return false;
}
        ...
        mSelected(false)
{
    if (parentPlot) {
        ...
        mFont =
            QFont(parentPlot->font().family(), parentPlot->font().pointSize() * 1.5, QFont::Bold);
        mSelectedFont =
            QFont(parentPlot->font().family(), parentPlot->font().pointSize() * 1.6, QFont::Bold);
    }
...
void QCPColorScale::setType(QCPAxis::AxisType type)
{
    ...
    if (mType != type) {
        ...
        connect(
            mColorAxis.data(), SIGNAL(rangeChanged(QCPRange)), this, SLOT(setDataRange(QCPRange)));
 ...
 */
QCPData::QCPData() :
        key(0), value(0), keyErrorPlus(0), keyErrorMinus(0), valueErrorPlus(0), valueErrorMinus(0)
{
}
...
void QCPGraph::drawLegendIcon(QCPPainter* painter, const QRectF& rect) const
{
    ...
    if (mBrush.style() != Qt::NoBrush) {
        ...
        painter->fillRect(
            QRectF(
                rect.left(), rect.top() + rect.height() / 2.0, rect.width(), rect.height() / 3.0),
            mBrush);
    }
                                  ...
                                  QVector<QCPData>* scatterData) const
{
    ...
    linePixelData->resize(
        lineData.size() * 2); // no need to reserve 2 extra points because impulse plot has no fill
...
void QCPGraph::getPreparedData(QVector<QCPData>* lineData, QVector<QCPData>* scatterData) const
{
    ...
    if (mAdaptiveSampling) {
        int keyPixelSpan =
            qAbs(keyAxis->coordToPixel(lower.key()) - keyAxis->coordToPixel(upper.key()));
            ...
            >= maxCount) // use adaptive sampling only if there are at least two points per pixel on average
    {
        if (lineData) {
            ...
            double currentIntervalStartKey =
                keyAxis->pixelToCoord((int) (keyAxis->coordToPixel(lower.key()) + reversedRound));
                                    ...
                                    QCPDataMap::const_iterator& upper) const
{
    ...
    bool lowoutlier =
        lbound != mData->constBegin(); // indicates whether there exist points below axis range
    bool highoutlier =
        ubound != mData->constEnd(); // indicates whether there exist points above axis range
        ...
        if (keyAxis->orientation() == Qt::Vertical) {
            if ((valueAxis->range().upper < 0 && !valueAxis->rangeReversed())
                || (valueAxis->range().upper > 0
                    && valueAxis
                           ->rangeReversed())) // if range is negative, zero is on opposite side of key axis
...
double QCPGraph::pointDistance(const QPointF& pixelPoint) const
{
    ...
    if (mData->size() == 1) {
        QPointF dataPoint =
            coordsToPixels(mData->constBegin().key(), mData->constBegin().value().value);
    ...
    if (mLineStyle == lsNone) {
        ...
        QPointF ptB = coordsToPixels(
            ...
            scatterData->at(0)
                .value); // getScatterPlotData returns in plot coordinates, so transform to pixels
 ...
 */
QCPRange
QCPGraph::getValueRange(bool& foundRange, SignDomain inSignDomain, bool includeErrors) const
{
    QCPRange range;
    bool haveLower = false;
    bool haveUpper = false;
 ...
 */
        ...
        if (currentRegion == 5
            || (firstPoint
                && mBrush.style()
                    != Qt::
                        NoBrush)) // current is in R, add current and last if it wasn't added already
        {
            if (!addedLastAlready) // in case curve just entered R, make sure the last point outside R is also drawn correctly
                lineData->append(coordsToPixels(
                    (it - 1).value().key, (it - 1).value().value)); // add last point to vector
            else if (
                lastRegion
            ...
            lineData->append(
                coordsToPixels(it.value().key, it.value().value)); // add current point to vector
                 ...
                 != lastRegion) // changed region, add current and last if not added already
        {
            ...
            else // no special case that forbids optimized point placement, so do it:
            {
                ...
                if (!addedLastAlready)
                    lineData->append(outsideCoordsToPixels(
                        (it - 1).value().key, (it - 1).value().value, currentRegion, axisRect));
                // add current point, optimized:
                lineData->append(outsideCoordsToPixels(
                    it.value().key, it.value().value, currentRegion, axisRect));
            }
            addedLastAlready =
 ...
 */
QCPBars::QCPBars(QCPAxis* keyAxis, QCPAxis* valueAxis) : QCPAbstractPlottable(keyAxis, valueAxis)
{
...
double QCPBars::getBaseValue(double key, bool positive) const
{
    if (mBarBelow) {
        ...
        QCPBarDataMap::const_iterator itEnd =
            mBarBelow.data()->mData->upperBound(key + mWidth * 0.1);
        while (it != itEnd) {
...
QCPRange QCPBars::getKeyRange(bool& foundRange, SignDomain inSignDomain) const
{
    ...
    while (it != mData->constEnd()) {
        current = it.value().key;
        if (inSignDomain == sdBoth || (inSignDomain == sdNegative && current + barWidthHalf < 0)
            || (inSignDomain == sdPositive && current - barWidthHalf > 0)) {
...
/* inherits documentation from base class */
double
QCPStatisticalBox::selectTest(const QPointF& pos, bool onlySelectable, QVariant* details) const
{
    Q_UNUSED(details)
    if (onlySelectable && !mSelectable)
        return -1;
...
QCPRange QCPStatisticalBox::getValueRange(bool& foundRange, SignDomain inSignDomain) const
{
    ...
    for (int i = 0; i < values.size(); ++i) {
        if ((inSignDomain == sdNegative && values.at(i) < 0)
            || (inSignDomain == sdPositive && values.at(i) > 0) || (inSignDomain == sdBoth)) {
...
double QCPColorMapData::data(double key, double value)
{
    ...
    int valueCell = (1.0 - (value - mValueRange.lower) / (mValueRange.upper - mValueRange.lower))
            * (mValueSize - 1)
        + 0.5;
...
void QCPColorMapData::setData(double key, double value, double z)
{
    int keyCell =
        (key - mKeyRange.lower) / (mKeyRange.upper - mKeyRange.lower) * (mKeySize - 1) + 0.5;
    int valueCell =
        (value - mValueRange.lower) / (mValueRange.upper - mValueRange.lower) * (mValueSize - 1)
        + 0.5;
    if (keyCell >= 0 && keyCell < mKeySize && valueCell >= 0 && valueCell < mValueSize) {
...
void QCPColorMap::updateMapImage()
{
    ...
    if (keyAxis->orientation() == Qt::Horizontal
        && (mMapImage.size().width() != mMapData->keySize()
            || mMapImage.size().height() != mMapData->valueSize()))
        mMapImage = QImage(QSize(mMapData->keySize(), mMapData->valueSize()), QImage::Format_RGB32);
    else if (keyAxis->orientation() == Qt::Vertical
             && (mMapImage.size().width() != mMapData->valueSize()
                 || mMapImage.size().height() != mMapData->keySize()))
...
void QCPColorMap::draw(QCPPainter* painter)
{
    ...
    if (mMapData->valueSize() > 1)
        halfSampleValue =
            0.5 * mMapData->valueRange().size() / (double) (mMapData->valueSize() - 1);
    QRectF imageRect(coordsToPixels(mMapData->keyRange().lower - halfSampleKey,
                                    mMapData->valueRange().lower - halfSampleValue),
                     coordsToPixels(mMapData->keyRange().upper + halfSampleKey,
                                    mMapData->valueRange().upper + halfSampleValue));
...
/* inherits documentation from base class */
double
QCPItemStraightLine::selectTest(const QPointF& pos, bool onlySelectable, QVariant* details) const
{
    Q_UNUSED(details)
    if (onlySelectable && !mSelectable)
        return -1;
...
QCPItemLine::QCPItemLine(QCustomPlot* parentPlot) :
        QCPAbstractItem(parentPlot), start(createPosition("start")), end(createPosition("end"))
{
    start->setCoords(0, 0);
    end->setCoords(1, 1);
...
void QCPItemLine::draw(QCPPainter* painter)
{
    ...
    QLineF line = getRectClippedLine(
        startVec, endVec, clipRect().adjusted(-clipPad, -clipPad, clipPad, clipPad));
    // paint visible segment, if existent:
    if (!line.isNull()) {
        painter->setPen(mainPen());
        painter->drawLine(line);
...
void QCPItemCurve::draw(QCPPainter* painter)
{
    ...
    // paint visible segment, if existent:
    QRect clip = clipRect().adjusted(
        -mainPen().widthF(), -mainPen().widthF(), mainPen().widthF(), mainPen().widthF());
    ...
    if (clip.intersects(cubicRect)) {
        ...
        if (mTail.style() != QCPLineEnding::esNone)
            mTail.draw(
                painter, QVector2D(startVec), M_PI - cubicPath.angleAtPercent(0) / 180.0 * M_PI);
        if (mHead.style() != QCPLineEnding::esNone)
...
void QCPItemPixmap::updateScaledPixmap(QRect finalRect, bool flipHorz, bool flipVert)
{
    ...
    if (mScaled) {
        ...
        if (finalRect.size() != mScaledPixmap.size()) {
            mScaledPixmap =
                mPixmap.scaled(finalRect.size(), mAspectRatioMode, Qt::SmoothTransformation);
...
QCPItemTracer::QCPItemTracer(QCustomPlot* parentPlot) :
        QCPAbstractItem(parentPlot), position(createPosition("position")), mGraph(0)
{
    position->setCoords(0, 0);
...
double QCPItemTracer::selectTest(const QPointF& pos, bool onlySelectable, QVariant* details) const
{
    ...
    case tsCrosshair: {
        return qSqrt(qMin(
            ...
            distSqrToLine(
                QPointF(center.x(), clip.top()), QPointF(center.x(), clip.bottom()), pos)));
    }
    case tsCircle: {
