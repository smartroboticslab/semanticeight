...
extern int processAll(se::Reader* reader,
                      bool processFrame,
                      bool renderImages,
...
static void newDenseSLAMSystem(bool resetPose)
{
    ...
    if (!resetPose) {
        *pipeline_pp =
            new DenseSLAMSystem(Eigen::Vector2i(640 / config->sensor_downsampling_factor,
...
CameraState setEnableCamera(CameraState state, string inputFile)
{
    ...
    if (state == CAMERA_RUNNING || state == CAMERA_LIVE) {
        if (reader == NULL || (reader->camera_open_ == false) || (state == CAMERA_LIVE)) {
            ...
            if (reader == NULL) {
                ...
                if (reader) {
                    ...
                    appWindow->setCameraFunction(&(reader->camera_active_),
                                                 (CameraState(*)(CameraState, std::string))
                                                     & setEnableCamera);
                }
                else {
                    bool camera_open_ = false;
                    appWindow->setCameraFunction(&camera_open_,
                                                 (CameraState(*)(CameraState, std::string))
                                                     & setEnableCamera);
                }
                *reader_pp = reader;
...
void qtIdle(void)
{
    ...
    if ((*reader_pp) && (*reader_pp)->camera_active_) {
        ...
        if ((*reader_pp == NULL) || !(*reader_pp)->camera_open_ || !(*reader_pp)->camera_active_)
            if (forceRender) {
    ...
    appWindow->update(*reader_pp != NULL ? (*reader_pp)->frame() + 1 : 0,
                      ((*reader_pp == NULL) || !((*reader_pp)->camera_active_))
                          ? CAMERA_CLOSED
                          : ((*reader_pp)->camera_open_
                                 ? ((config->sequence_path == "") ? CAMERA_LIVE : CAMERA_RUNNING)
                                 : CAMERA_PAUSED));
}
...
void dumpMesh()
{
    ...
    std::string filename =
        appWindow->fileSaveSelector("Save mesh", ".", "mesh (*.vtk);; All files (*.*)");
    if (filename != "")
...
{
    std::string filename =
        appWindow->fileSaveSelector("Save sequence log", ".", "*.log (*.log);; All files (*.*)");
    if (filename != "") {
...
{
    std::string filename =
        appWindow->fileSaveSelector("Save power log", ".", "log (*.prpt);; All files (*.*)");
...
void qtLinkKinectQt(int argc,
                    char* argv[],
                    DenseSLAMSystem** _pipe,
                    se::Reader** _depthReader,
                    se::Configuration* _config,
                    void* depthRender,
                    void* trackRender,
                    void* volumeRender,
                    void* RGBARender)
{
    ...
    bool camera_active_ =
        false; //this is a bodge as we might not have a reader yet therefore camera must be off

    appWindow->setCameraFunction((*reader_pp) ? &((*reader_pp)->camera_active_) : &camera_active_,
    ...
    appWindow->addButtonChoices("Vol. Size",
                                {"4.0mx4.0mx4.0m", "2.0mx2.0mx2.0m", "1.0mx1.0mx1.0m"},
                                {4.0, 2.0, 1.0},
                                (float*) (&(config->map_dim.x())),
                                continueWithNewDenseSLAMSystem);
    appWindow->addButtonChoices(
        "Vol. Res",
        {"1024x1024x1024", "512x512x512", "256x256x256", "128x128x128", "64x64x64", "32x32x32"},
        {1024, 512, 256, 128, 64, 32},
        (int*) &(config->map_size.x()),
        continueWithNewDenseSLAMSystem);

    appWindow->addButtonChoices("ICP threshold",
                                {"1e-4", "1e-5", "1e-6"},
                                {1e-4, 1e-5, 1e-6},
                                (float*) &(config->icp_threshold));

    int cwidth = (((*reader_pp) == NULL) ? 640 : ((*reader_pp)->depthImageRes()).x())
        / config->sensor_downsampling_factor;
    int cheight = (((*reader_pp) == NULL) ? 480 : ((*reader_pp)->depthImageRes()).y())
        / config->sensor_downsampling_factor;
    int width = (((*reader_pp) == NULL) ? 640 : ((*reader_pp)->depthImageRes()).x());
    int height = (((*reader_pp) == NULL) ? 480 : ((*reader_pp)->depthImageRes()).y());

    FImage rgbImage = {cwidth, cheight, GL_RGBA, GL_UNSIGNED_BYTE, RGBARender};
    FImage depthImage = {cwidth, cheight, GL_RGBA, GL_UNSIGNED_BYTE, depthRender};
    FImage trackImage = {cwidth, cheight, GL_RGBA, GL_UNSIGNED_BYTE, trackRender};
    ...
    FImage volumeImage = {config->render_volume_fullsize ? width : cwidth,
                          config->render_volume_fullsize ? height : cheight,
                          GL_RGBA,
                          GL_UNSIGNED_BYTE,
                          volumeRender};
    appWindow->viewers->addViewer(rgbImage,
                                  (const char*) "RGB image",
                                  (const char*) "RGB representation of input, unused in processing",
                                  NULL,
                                  false);
    appWindow->viewers->addViewer(
        depthImage,
    ...
    appWindow->viewers->addViewer(
        ...
        NULL,
        true);
    appWindow->viewers->addViewer(volumeImage,
                                  (const char*) "3D model",
                                  (const char*) "3D model",
                                  NULL,
    ...
    appWindow->viewers->addViewer(&se::perfstats,
                                  (const char*) "Performance",
                                  (const char*) "Performance Statistics",
                                  &statsEnabled);
    appWindow->viewers->setStatEntry(
        "Performance", {"X", "Y", "Z", "tracked", "integrated", "frame"}, false);
