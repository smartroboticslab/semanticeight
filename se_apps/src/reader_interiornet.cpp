...
struct InteriorNetPoseEntry
{
  ...
  InteriorNetPoseEntry(const std::string& s)
  {
    ...
    position = Eigen::Vector3f(
            std::stof(columns[10]), std::stof(columns[11]), std::stof(columns[12]));
    Eigen::Quaternionf q_WB = Eigen::Quaternionf(std::stof(columns[6]), std::stof(columns[7]), std::stof(columns[8]), std::stof(columns[9]));
  ...
  {
    const std::string s = std::to_string(timestamp) + " "
                          + rgb_filename + " "
                          + depth_filename + " "
  ...
  for (std::string line; std::getline(fs, line);)
  {
    ...
    if (!str_utils::is_float(columns[0], false))
    {
      std::cerr << "Error: Invalid timestamp in data line " << poses.size() + 1 << " of "
                << filename << "\n";
      std::cerr << "Error: The InteriorNet ground truth timestamps must be non-negative floats\n";
      poses.clear();
      return poses;
    }
...
se::InteriorNetReader::InteriorNetReader(const se::ReaderConfig& c) : se::Reader(c)
{
  ...
  if (!ground_truth_file_.empty())
  {
    ...
    std::vector<InteriorNetPoseEntry> gt_poses = read_interiornet_ground_truth(ground_truth_file_);
    if (gt_poses.empty())
    {
      status_ = se::ReaderStatus::error;
      camera_active_ = false;
    ...
    {
      std::cerr << "Error: Could not read generated ground truth file "
                << generated_filename << "\n";
      status_ = se::ReaderStatus::error;
      camera_active_ = false;
      camera_open_ = false;
  ...
  std::transform(depth_images.begin(), depth_images.end(),
                 depth_filenames_.begin(), [](const auto& e) { return e.filename; });
  // Get the filenames of the RGB images.
  rgb_filenames_.resize(rgb_images.size());
  std::transform(rgb_images.begin(), rgb_images.end(),
                 rgb_filenames_.begin(), [](const auto& e) { return e.filename; });
...
se::ReaderStatus se::InteriorNetReader::nextSegmentation(se::SegmentationResult& segmentation) {
  segmentation = se::SegmentationResult(rgba_image_res_.x(), rgba_image_res_.y());
  return se::ReaderStatus::ok;
}