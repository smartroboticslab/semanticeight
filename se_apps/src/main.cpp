...
struct ProgressBar {
    ...
    void update(int curr_frame)
    {
        if (total_frames_ == -1) {
            // "\033[K" clear line
            std::cout << "\033[K"
                      << "Processed frame " << std::setfill(' ') << std::setw(4) << curr_frame
                      << " of whole sequence"
                         "\n";
            ...
            // "\033[K" clear line
            std::cout << "\033[K"
                      << "Processed frame " << curr_frame << " of " << total_frames_ << "\n";
            std::stringstream ss;
            ss << std::setfill(' ') << std::setw(3) << percent;
            std::string progress = ss.str() + " % [" + std::string(percent, '*')
...
int main(int argc, char** argv)
{
    ...
    pipeline = new DenseSLAMSystem(image_res,
               ...
               bool reset)
{
    ...
    if (render_images) {
        ...
        if (frame == config->max_frame) {
            ...
            render_volume = (config->rendering_rate < 0) ? frame == std::abs(config->rendering_rate)
                                                         : frame % config->rendering_rate == 0;
        }
        ...
        if (render_volume) {
            pipeline->renderObjects(volume_render,
                                    pipeline->getImageResolution(),
                                    sensor,
                                    RenderMode::InstanceID,
                                    false);
            pipeline->renderObjects(volume_render_color,
                                    pipeline->getImageResolution(),
                                    sensor,
                                    RenderMode::Color,
                                    false);
            pipeline->renderObjects(volume_render_scale,
                                    pipeline->getImageResolution(),
                                    sensor,
                                    RenderMode::Scale,
                                    false);
            pipeline->renderObjects(volume_render_min_scale,
                                    pipeline->getImageResolution(),
                                    sensor,
                                    RenderMode::MinScale,
                                    false);
    ...
    if (render_volume && config->output_render_file != "") {
        ...
        pipeline->renderInputSegmentation(segmentation_render.get(),
                                          pipeline->getImageResolution());
        std::unique_ptr<uint32_t[]> volume_aabb_render(new uint32_t[w * h]);
        pipeline->renderObjects(volume_aabb_render.get(),
                                pipeline->getImageResolution(),
                                sensor,
                                RenderMode::InstanceID);

        lodepng_encode32_file(
            (prefix + "rgba_" + suffix).c_str(), (unsigned char*) rgba_render, w, h);
        lodepng_encode32_file(
            (prefix + "depth_" + suffix).c_str(), (unsigned char*) depth_render, w, h);
        lodepng_encode32_file(
            (prefix + "segm_" + suffix).c_str(), (unsigned char*) segmentation_render.get(), w, h);
        lodepng_encode32_file(
            (prefix + "volume_" + suffix).c_str(), (unsigned char*) volume_render, w, h);
        lodepng_encode32_file((prefix + "volume_color_" + suffix).c_str(),
                              (unsigned char*) volume_render_color,
                              w,
                              h);
        lodepng_encode32_file((prefix + "volume_scale_" + suffix).c_str(),
                              (unsigned char*) volume_render_scale,
                              w,
                              h);
        lodepng_encode32_file((prefix + "volume_min_scale_" + suffix).c_str(),
                              (unsigned char*) volume_render_min_scale,
                              w,
                              h);
        lodepng_encode32_file((prefix + "volume_aabb_" + suffix).c_str(),
                              ...
                              h);
        lodepng_encode32_file(
            (prefix + "raycast_" + suffix).c_str(), (unsigned char*) raycast_render, w, h);
        lodepng_encode32_file(
            (prefix + "instance_" + suffix).c_str(), (unsigned char*) instance_render, w, h);
        lodepng_encode32_file(
            (prefix + "class_" + suffix).c_str(), (unsigned char*) class_render, w, h);
    }
    ...
    if (config->enable_meshing) {
        if (frame == config->max_frame) {
            ...
            mesh_volume = (config->meshing_rate < 0) ? frame == std::abs(config->meshing_rate)
                                                     : frame % config->meshing_rate == 0;
        }
    }
