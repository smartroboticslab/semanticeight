...
se::Reader* se::create_reader(const Configuration& config)
{
    ...
    const se::ReaderConfig reader_config = {config.fps,
                                            config.drop_frames,
    ...
    if (config.sequence_type == "openni"
        && (reader_config.sequence_path.empty()
    ...
    else if (config.sequence_type == "interiornet"
             && stdfs::is_directory(reader_config.sequence_path)) {
        reader = new se::InteriorNetReader(reader_config);
    }
    else {
