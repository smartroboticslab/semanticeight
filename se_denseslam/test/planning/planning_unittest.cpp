...
struct Pose3D {
  ...
  int print(FILE* f = stdout) const {
    return fprintf(f, "t: % 8.3f % 8.3f % 8.3f   q: % 6.3f % 6.3f % 6.3f % 6.3f\n",
...
VecPose addPathSegments(const Pose3D& start_in,
  ...
  if (candidate_path.size() > 2) {
    for (size_t i = 1; i < candidate_path.size(); i++) {
      ...
      if ((candidate_path[i].p - candidate_path[i - 1].p).norm() > v_max * dt) {
        path_tmp = addPathSegments(candidate_path[i-1], candidate_path[i], dt, v_max, res);
      } else {
...
TEST_F(ICRA2020Planning, addIntermediateTranslation) {
  VecPose old_tran_path = addPathSegments(old_start_, old_end_, delta_t_, linear_velocity_, resolution_);
  se::Path new_tran_path = se::CandidateView::addIntermediateTranslation(new_start_, new_end_, delta_t_, linear_velocity_, resolution_);
...
TEST_F(ICRA2020Planning, addIntermediateYaw) {
  ...
  se::Path new_yaw_path = se::CandidateView::addIntermediateYaw(new_start_, new_end_, delta_t_, angular_velocity_);
...
TEST_F(ICRA2020Planning, getFinalPath) {
  const VecPose old_final_path = getFinalPath(old_start_, old_end_, old_path_, delta_t_,
      linear_velocity_, angular_velocity_, resolution_);
  const se::Path new_final_path = se::CandidateView::getFinalPath(new_path_, delta_t_,
      linear_velocity_, angular_velocity_, resolution_);
