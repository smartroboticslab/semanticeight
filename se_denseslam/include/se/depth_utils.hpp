...
namespace se {
   ...
   */
  inline float depth_image_error(const cv::Mat& depth_1,
                                 const cv::Mat& depth_2,
                                 const cv::Mat& mask) {
    ...
    assert((depth_1.cols == depth_2.cols)
        && "Error: depth_1 and depth_2 must have the same width");
    assert((depth_1.rows == depth_2.rows)
        && "Error: depth_1 and depth_2 must have the same height");
    assert((depth_1.cols == mask.cols)
        && "Error: depth_1 and mask must have the same width");
    assert((depth_1.rows == mask.rows)
        && "Error: depth_1 and mask must have the same height");
  ...
  inline float depth_image_error(const se::Image<float>& depth_1,
    ...
    // Mats are const.
    const cv::Mat depth_1_cv (depth_1.height(),
        depth_1.width(),
        CV_32FC1,
        const_cast<float*>(depth_1.data()));
    const cv::Mat depth_2_cv (depth_2.height(),
        depth_2.width(),
        CV_32FC1,
        const_cast<float*>(depth_2.data()));
