...
struct hash<Eigen::Vector3d> {
    ...
    {
        return (std::hash<double>()(vector.x()) ^ ((std::hash<double>()(vector.y()) << 1) >> 1)
                ^ (std::hash<double>()(vector.z()) << 1));
    }
};
...
typedef std::vector<Eigen::Quaternionf, Eigen::aligned_allocator<Eigen::Quaternionf>>
    VectorQuaternionf;
...
inline bool
CloseEnough(const Eigen::Vector3d& v1, const Eigen::Vector3d& v2, const double threshold)
{
...
inline Eigen::MatrixXd ClampNorm(const Eigen::MatrixBase<Derived>& item_to_clamp,
...
template<typename Container>
inline std::pair<Container, uint64_t> Deserialize(const std::vector<uint8_t>& buffer,
...
{
    (void) (value);
    return (uint64_t)((1 * sizeof(uint64_t))
                      + ((size_t) value.size()
                         * sizeof(double))); // Space for a uint64_t size header and the data
}
...
inline uint64_t Serialize(const Eigen::VectorXd& value, std::vector<uint8_t>& buffer)
{
    ...
    // Copy the data
    memcpy(
        &(temp_buffer[sizeof(size_header)]), value.data(), (serialized_size - sizeof(size_header)));
    buffer.insert(buffer.end(), temp_buffer.begin(), temp_buffer.end());
    return serialized_size;
}

template<>
inline std::pair<Eigen::VectorXd, uint64_t>
Deserialize<Eigen::VectorXd>(const std::vector<uint8_t>& buffer, const uint64_t current)
{
    ...
    memcpy(temp_value.data(),
           &buffer[current + sizeof(size_header)],
...
template<>
inline std::pair<Eigen::Vector3d, uint64_t>
Deserialize<Eigen::Vector3d>(const std::vector<uint8_t>& buffer, const uint64_t current)
{
    assert(current < buffer.size());
...
template<>
inline std::pair<Eigen::Matrix<double, 6, 1>, uint64_t>
Deserialize<Eigen::Matrix<double, 6, 1>>(const std::vector<uint8_t>& buffer, const uint64_t current)
{
    assert(current < buffer.size());
...
template<>
inline std::pair<Eigen::Affine3d, uint64_t>
Deserialize<Eigen::Affine3d>(const std::vector<uint8_t>& buffer, const uint64_t current)
{
    assert(current < buffer.size());
...
inline Eigen::Vector3d RotateVectorReverse(const Eigen::Quaterniond& quat,
...
inline std::vector<double> Multiply(const std::vector<double>& vec1,
...
inline Eigen::Matrix3d Skew(const Eigen::Vector3d& vector)
{
    ...
    skewed << 0.0, -vector.z(), vector.y(), vector.z(), 0.0, -vector.x(), -vector.y(), vector.x(),
        0.0;
    return skewed;
}
...
inline Eigen::Vector3d Unskew(const Eigen::Matrix3d& matrix)
{
    ...
    const Eigen::Vector3d unskewed(
        matrix_symetric(2, 1), matrix_symetric(0, 2), matrix_symetric(1, 0));
    return unskewed;
}
...
inline Eigen::Matrix<double, 6, 1> TransformTwist(const Eigen::Affine3d& transform,
                                                  const Eigen::Matrix<double, 6, 1>& initial_twist)
{
    return (Eigen::Matrix<double, 6, 1>) (EigenHelpers::AdjointFromTransform(transform)
                                          * initial_twist);
}
...
inline Eigen::Matrix3d ExpMatrixExact(const Eigen::Matrix3d& hatted_rot_velocity,
...
inline Eigen::Affine3d ExpTwist(const Eigen::Matrix<double, 6, 1>& twist, const double delta_t)
{
    ...
    if (rot_velocity_norm >= 1e-100) {
        ...
        const Eigen::Vector3d translation_displacement =
            ...
            + (scaled_rot_velocity * scaled_rot_velocity.transpose() * scaled_trans_velocity
               * scaled_delta_t);
        ...
        if ((trans_velocity_norm >= 1e-100) || (rot_velocity_norm == 0.0)) {
            ...
            std::cerr
                << "*** WARNING - YOU MAY ENCOUNTER NUMERICAL INSTABILITY IN EXPTWIST(...) WITH TRANS & ROT VELOCITY NORM < 1e-100 ***"
                << std::endl;
            const double scaled_delta_t = delta_t * rot_velocity_norm;
            const Eigen::Vector3d scaled_trans_velocity = trans_velocity / rot_velocity_norm;
            const Eigen::Vector3d scaled_rot_velocity = rot_velocity / rot_velocity_norm;
...
Interpolate(const std::vector<double>& v1, const std::vector<double>& v2, const double ratio)
{
    // Safety check ratio
    double real_ratio = ratio;
    if (real_ratio < 0.0) {
    ...
    if (len != v2.size()) {
        std::cerr << "Vectors to interpolate are different sizes (" << v1.size() << " versus "
                  << v2.size() << ")" << std::endl;
        return std::vector<double>();
    }
    // Interpolate
...
Interpolate(const Eigen::Quaterniond& q1, const Eigen::Quaterniond& q2, const double ratio)
{
    // Safety check ratio
    double real_ratio = ratio;
    if (real_ratio < 0.0) {
...
Interpolate(const Eigen::Vector3d& v1, const Eigen::Vector3d& v2, const double ratio)
{
    // Safety check ratio
    double real_ratio = ratio;
    if (real_ratio < 0.0) {
...
Interpolate(const Eigen::Affine3d& t1, const Eigen::Affine3d& t2, const double ratio)
{
    // Safety check ratio
    double real_ratio = ratio;
    if (real_ratio < 0.0) {
...
{
    const double dq =
        fabs((q1.w() * q2.w()) + (q1.x() * q2.x()) + (q1.y() * q2.y()) + (q1.z() * q2.z()));
...
inline double
Distance(const Eigen::Affine3d& t1, const Eigen::Affine3d& t2, const double alpha = 0.5)
{
    assert(alpha >= 0.0);
    assert(alpha <= 1.0);
    const Eigen::Vector3d v1 = t1.translation();
...
inline Eigen::Affine3d TransformFromRPY(const double x,
                                        const double y,
                                        const double z,
                                        const double roll,
                                        const double pitch,
...
inline Eigen::Affine3d TransformFromRPY(const Eigen::Vector3d& translation,
                                        const Eigen::Vector3d& rotation)
{
    const Eigen::Affine3d transform = (Eigen::Translation3d) translation
...
inline Eigen::Affine3d TransformFromRPY(const Eigen::VectorXd& components)
{
    ...
    const Eigen::Affine3d transform =
        Eigen::Translation3d(components(0), components(1), components(2))
...
inline Eigen::Matrix<ScalarType, Rows, 1>
AverageEigenVector(const std::vector<Eigen::Matrix<ScalarType, Rows, 1>, Allocator>& vectors,
                   const std::vector<double>& weights = std::vector<double>())
{
    // Get the weights
...
inline Eigen::Vector3d
AverageEigenVector3d(const EigenHelpers::VectorVector3d& vectors,
                     const std::vector<double>& weights = std::vector<double>())
{
...
AverageEigenVectorXd(const std::vector<Eigen::VectorXd>& vectors,
                     const std::vector<double>& weights = std::vector<double>())
{
    return AverageEigenVector(vectors, weights);
}
...
/**
     ...
     */
inline Eigen::Quaterniond
AverageEigenQuaterniond(const EigenHelpers::VectorQuaterniond& quaternions,
                        const std::vector<double>& weights = std::vector<double>())
{
    // Get the weights
    ...
    for (size_t idx = 0; idx < quaternions.size(); idx++) {
        ...
        q_matrix.col((ssize_t) idx) << weight * q.w(), weight * q.x(), weight * q.y(),
            weight * q.z();
    }
    ...
    const Eigen::EigenSolver<Eigen::Matrix<double, 4, 4>>::EigenvalueType eigen_values =
        solver.eigenvalues();
    const Eigen::EigenSolver<Eigen::Matrix<double, 4, 4>>::EigenvectorsType eigen_vectors =
        solver.eigenvectors();
...
inline Eigen::Affine3d
AverageEigenAffine3d(const EigenHelpers::VectorAffine3d& transforms,
                     const std::vector<double>& weights = std::vector<double>())
{
    ...
    const Eigen::Affine3d average_transform =
...
VectorProjection(const Eigen::MatrixBase<DerivedB>& base_vector,
                 const Eigen::MatrixBase<DerivedV>& vector_to_project)
{
    ...
    if (b_squared_norm > 0) {
        ...
        return Eigen::Matrix<typename DerivedB::Scalar, Eigen::Dynamic, 1>::Zero(
            base_vector.rows());
    }
}
...
template<typename DerivedB, typename DerivedV>
inline Eigen::Matrix<typename DerivedB::Scalar, Eigen::Dynamic, 1>
VectorRejection(const Eigen::MatrixBase<DerivedB>& base_vector,
                const Eigen::MatrixBase<DerivedV>& vector_to_project)
{
...
inline double WeightedDotProduct(const Eigen::VectorXd& vec1,
...
inline double WeightedCosineAngleBetweenVectors(const Eigen::VectorXd& vec1,
                                                const Eigen::VectorXd& vec2,
                                                const Eigen::VectorXd& weights)
{
...
inline double WeightedAngleBetweenVectors(const Eigen::VectorXd& vec1,
                                          const Eigen::VectorXd& vec2,
                                          const Eigen::VectorXd& weights)
{
...
class Hyperplane {
    ...
    Eigen::VectorXd ProjectPointOntoPlane(const Eigen::VectorXd& point) const
    {
        ...
        const Eigen::VectorXd projected_to_point_vector =
            VectorRejection(plane_normal_, origin_to_point_vector);
...
inline Hyperplane FitPlaneToPoints(const std::vector<Eigen::VectorXd>& points)
{
    ...
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(centered_points,
...
Pinv(const Eigen::MatrixXd& b, const double rcond, const bool enable_flip = true)
{
    bool flip = false;
    Eigen::MatrixXd a;
...
/**
     ...
     * @return size N x 1
     */
inline Eigen::VectorXd WeightedLeastSquaresSolver(const Eigen::MatrixXd& A,
                                                  const Eigen::VectorXd& b,
                                                  const Eigen::VectorXd& w,
                                                  const double damping_threshold,
