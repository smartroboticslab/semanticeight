// Taken from https://github.com/UM-ARM-Lab/sdf_tools.
// Modified by Marius Grimm (marius.grimm93@web.de)
...
inline Value RetrieveOrDefault(const std::map<Key, Value, Compare, Allocator>& map,
                               const Key& key,
...
template<typename Key,
         ...
         typename Allocator = std::allocator<std::pair<const Key, Value>>>
inline Value
RetrieveOrDefault(const std::unordered_map<Key, Value, Hash, Predicate, Allocator>& map,
                  const Key& key,
                  const Value& default_val)
{
    const auto found_itr = map.find(key);
...
inline T ClampValueAndWarn(const T& val, const T& min, const T& max)
{
    ...
    if (val < min) {
        const std::string msg =
            "Clamping " + std::to_string(val) + " to min " + std::to_string(min) + "\n";
        std::cerr << msg << std::flush;
        return min;
    }
    else if (val > max) {
        const std::string msg =
            "Clamping " + std::to_string(val) + " to max " + std::to_string(max) + "\n";
        std::cerr << msg << std::flush;
        return max;
    }
    return val;
...
template<class T>
inline T
SpreadValue(const T& val, const T& low_threshold, const T& midpoint, const T& high_threshold)
{
...
template<class T>
inline T
SpreadValueAndWarn(const T& val, const T& low_threshold, const T& midpoint, const T& high_threshold)
{
    assert(low_threshold <= midpoint);
    assert(midpoint <= high_threshold);
    if (val >= midpoint && val < high_threshold) {
        const std::string msg = "Thresholding " + std::to_string(val) + " to high threshold "
            + std::to_string(high_threshold) + "\n";
        std::cerr << msg << std::flush;
        return high_threshold;
    }
    else if (val < midpoint && val > low_threshold) {
        const std::string msg = "Thresholding " + std::to_string(val) + " to low threshold "
...
class RGBAColor {
    ...
    RGBAColor(const float r, const float g, const float b, const float a) :
            r(TrimColorValue(r)), g(TrimColorValue(g)), b(TrimColorValue(b)), a(TrimColorValue(a))
    {
    }

    RGBAColor(const float r, const float g, const float b) :
            r(TrimColorValue(r)), g(TrimColorValue(g)), b(TrimColorValue(b)), a(1.0f)
    {
    }

    RGBAColor(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a) :
            r(ColorChannelFromHex(r)),
            g(ColorChannelFromHex(g)),
            b(ColorChannelFromHex(b)),
            a(ColorChannelFromHex(a))
    {
    }

    RGBAColor(const uint8_t r, const uint8_t g, const uint8_t b, const float a) :
            r(ColorChannelFromHex(r)),
            g(ColorChannelFromHex(g)),
            b(ColorChannelFromHex(b)),
            a(TrimColorValue(a))
...
class RGBAColorBuilder {
    ...
    public:
    static inline ColorType
    MakeFromFloatColors(const float r, const float g, const float b, const float a = 1.0f)
    {
        ColorType color;
    ...
    static inline ColorType
    MakeFromHexColors(const uint8_t r, const uint8_t g, const uint8_t b, const uint8_t a = 0xff)
    {
        return MakeFromFloatColors(ColorChannelFromHex(r),
                                   ColorChannelFromHex(g),
                                   ColorChannelFromHex(b),
                                   ColorChannelFromHex(a));
    }

    static inline ColorType
    MakeFromMixedColors(const uint8_t r, const uint8_t g, const uint8_t b, const float a = 1.0f)
    {
        return MakeFromFloatColors(ColorChannelFromHex(r),
                                   ColorChannelFromHex(g),
                                   ColorChannelFromHex(b),
    ...
    static inline ColorType InterpolateHotToCold(const double value,
...
BuildDistanceMatrix(const std::vector<Datatype, Allocator>& data,
...
template<typename FirstDatatype,
         typename SecondDatatype,
         typename FirstAllocator = std::allocator<FirstDatatype>,
         typename SecondAllocator = std::allocator<SecondDatatype>>
inline Eigen::MatrixXd BuildDistanceMatrix(
    const std::vector<FirstDatatype, FirstAllocator>& data1,
    const std::vector<SecondDatatype, SecondAllocator>& data2,
...
GetKNearestNeighbors(const std::vector<Item, ItemAlloc>& items,
                     const Value& current,
                     ...
                     const size_t K)
{
    ...
    if (items.size() > K) {
        std::function<bool(const std::pair<int64_t, double>&, const std::pair<int64_t, double>&)>
            compare_fn = [](const std::pair<int64_t, double>& index1,
...
#    endif
            ...
            auto itr = std::max_element(
                current_thread_nearests.begin(), current_thread_nearests.end(), compare_fn);
        ...
        for (size_t thread_idx = 0; thread_idx < per_thread_nearests.size(); thread_idx++) {
            const std::vector<std::pair<int64_t, double>>& thread_nearests =
                per_thread_nearests[thread_idx];
        ...
        std::vector<std::pair<int64_t, double>> k_nearests(
            items.size(), std::make_pair(-1, std::numeric_limits<double>::infinity()));
#    ifdef ENABLE_PARALLEL_K_NEAREST_NEIGHBORS
#        pragma omp parallel for
#    endif
        for (size_t idx = 0; idx < items.size(); idx++) {
...
inline double EvaluateTruncatedGaussianCDF(const double mean,
...
inline double EvaluateTruncatedGaussianPDF(const double mean,
                                           const double lower_bound,
                                           const double upper_bound,
                                           const double std_dev,
                                           const double val)
...
inline double IntegrateGaussian(const double mean,
                                const double std_dev,
...
inline double IntegrateTruncatedGaussian(const double mean,
                                         const double lower_bound,
                                         const double upper_bound,
                                         const double std_dev,
                                         const double lower_limit,
                                         const double upper_limit)
{
    ...
    const double lower_limit_cdf =
        EvaluateTruncatedGaussianCDF(mean, lower_bound, upper_bound, std_dev, lower_limit);
    const double upper_limit_cdf =
        EvaluateTruncatedGaussianCDF(mean, lower_bound, upper_bound, std_dev, upper_limit);
    const double probability = upper_limit_cdf - lower_limit_cdf;
...
class TruncatedNormalDistribution {
    ...
    inline bool CheckSimple(const double lower_bound, const double upper_bound) const
    {
        ...
        const double val2 =
            exp((pow(lower_bound, 2) - lower_bound * sqrt(pow(lower_bound, 2) + 4)) / (4));
    ...
    inline double
    NaiveAcceptReject(const double lower_bound, const double upper_bound, Generator& prng)
    {
        while (true) {
            const double draw = normal_dist_(
                prng); // Rf_rnorm(0.0, 1.0) ; // Normal distribution (i.e. std::normal_distribution<double>)
    ...
    inline double SimpleAcceptReject(const double lower_bound, Generator& prng)
    {
        ...
        while (true) {
            const double e = exponential_dist_(
                prng); // Rf_rexp(1.0) ; // Exponential distribution (i.e. std::exponential_distribution<double>)
            const double z = lower_bound + e / alpha;
            const double rho = exp(-pow(alpha - z, 2) / 2);
            const double u = uniform_unit_dist_(
                prng); //  Rf_runif(0, 1) ; // Uniform distribution (i.e. std::uniform_real_distribution<double>)
            if (u <= rho) {
                return z;
            }
        }
    ...
    template<typename Generator>
    inline double
    ComplexAcceptReject(const double lower_bound, const double upper_bound, Generator& prng)
    {
        while (true) {
            const double z = uniform_range_dist_(
                prng); // Rf_runif(lower_bound, upper_bound) ; // Uniform distribution (i.e. std::uniform_real_distribution<double>)
            double rho = 0.0;
            if (0 < lower_bound) {
    ...
    inline double Sample(Generator& prng)
    {
        if (case_ == TYPE_1) {
            ...
            while (true) {
                const double draw = SimpleAcceptReject(
                    std_lower_bound_, prng); // use the simple algorithm if it is more efficient
    ...
    public:
    inline TruncatedNormalDistribution(const double mean,
                                       const double stddev,
                                       const double lower_bound,
                                       const double upper_bound) :
            ...
            normal_dist_(0.0, 1.0)
    {
        ...
        if (fabs(stddev_) == 0.0) {
            ...
            if ((0.0 > std_upper_bound_ || 0.0 < std_lower_bound_)
                && !(std_upper_bound_ == INFINITY || std_lower_bound_ == -INFINITY)) {
            ...
            assert((case_ == TYPE_1) || (case_ == TYPE_2) || (case_ == TYPE_3)
                   || (case_ == TYPE_4));
        }
    }
...
class MultivariteGaussianDistribution {
    ...
    static Eigen::MatrixXd CalculateNormTransform(const Eigen::MatrixXd& covariance)
    {
        ...
        if (chol_solver.info() == Eigen::Success) {
            ...
            norm_transform = eigen_solver.eigenvectors()
    ...
    public:
    inline MultivariteGaussianDistribution(const Eigen::VectorXd& mean,
                                           const Eigen::MatrixXd& covariance) :
            mean_(mean),
            norm_transform_(CalculateNormTransform(covariance)),
            unit_gaussian_dist_(0.0, 1.0)
    {
        assert(mean.rows() == covariance.rows());
        assert(covariance.cols() == covariance.rows());

        assert(!(norm_transform_.unaryExpr([](const double& val) { return std::isnan(val); })).any()
               && "NaN Found in norm_transform in MultivariateGaussianDistribution");
        assert(!(norm_transform_.unaryExpr([](const double& val) { return std::isinf(val); })).any()
...
class RandomRotationGenerator {
    ...
    GenerateUniformRandomQuaternion(const std::function<double()>& uniform_unit_dist)
    {
    ...
    GenerateUniformRandomEulerAngles(const std::function<double()>& uniform_unit_dist)
    {
        const double roll = 2.0 * M_PI * uniform_unit_dist() - M_PI;
        const double pitch_init = std::acos(1.0 - (2.0 * uniform_unit_dist())) + M_PI_2;
        const double pitch = (uniform_unit_dist() < 0.5)
...
template<typename T>
inline std::pair<T, uint64_t> DeserializeFixedSizePOD(const std::vector<uint8_t>& buffer,
                                                      const uint64_t current);

template<typename T, typename Allocator = std::allocator<T>>
inline uint64_t
SerializeVector(const std::vector<T, Allocator>& vec_to_serialize,
...
inline std::pair<std::vector<T, Allocator>, uint64_t> DeserializeVector(
    ...
    const uint64_t current,
    const std::function<std::pair<T, uint64_t>(const std::vector<uint8_t>&, const uint64_t)>&
        item_deserializer);
...
inline uint64_t
SerializeMap(const std::map<Key, T, Compare, Allocator>& map_to_serialize,
             std::vector<uint8_t>& buffer,
...
inline std::pair<std::map<Key, T, Compare, Allocator>, uint64_t> DeserializeMap(
    const std::vector<uint8_t>& buffer,
    const uint64_t current,
    const std::function<std::pair<Key, uint64_t>(const std::vector<uint8_t>&, const uint64_t)>&
        key_deserializer,
    const std::function<std::pair<T, uint64_t>(const std::vector<uint8_t>&, const uint64_t)>&
        value_deserializer);
...
inline const std::pair<std::pair<First, Second>, uint64_t> DeserializePair(
    const std::vector<uint8_t>& buffer,
    const uint64_t current,
    const std::function<std::pair<First, uint64_t>(const std::vector<uint8_t>&, const uint64_t)>&
        first_deserializer,
...
inline uint64_t SerializeString(const std::basic_string<CharType>& str_to_serialize,
                                std::vector<uint8_t>& buffer)
{
    const uint64_t start_buffer_size = buffer.size();
...
DeserializeString(const std::vector<uint8_t>& buffer, const uint64_t current)
{
    ...
    const std::pair<uint64_t, uint64_t> deserialized_size =
        DeserializeFixedSizePOD<uint64_t>(buffer, current_position);
    ...
    for (uint64_t idx = 0; idx < size; idx++) {
        const std::pair<CharType, uint64_t> deserialized_char =
            DeserializeFixedSizePOD<CharType>(buffer, current_position);
...
inline void
ConditionalPrint(const std::string& msg, const int32_t msg_level, const int32_t print_level)
{
...
template<typename Key,
         ...
         typename Compare = std::less<Key>,
         typename Allocator = std::allocator<std::pair<const Key, Value>>>
inline std::vector<std::pair<const Key, Value>, Allocator>
GetKeysAndValues(const std::map<Key, Value, Compare, Allocator>& map)
{
...
template<typename Key,
         ...
         typename Compare = std::less<Key>,
         typename Allocator = std::allocator<std::pair<const Key, Value>>>
inline std::map<Key, Value, Compare, Allocator>
MakeFromKeysAndValues(const std::vector<std::pair<const Key, Value>, Allocator>& keys_and_values)
{
...
inline std::map<Key, Value, Compare, PairAllocator>
MakeFromKeysAndValues(const std::vector<Key, KeyVectorAllocator>& keys,
