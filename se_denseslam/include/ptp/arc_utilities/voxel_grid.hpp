...
class VoxelGrid {
    ...
    inline int64_t
    GetDataIndex(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
    ...
    {
        SafetyCheckSizes(
            cell_x_size, cell_y_size, cell_z_size, num_x_cells, num_y_cells, num_z_cells);
    ...
    Serialize(const VoxelGrid<T, Allocator>& grid,
    ...
    VoxelGrid(const Eigen::Affine3d& origin_transform,
    ...
    {
        Initialize(
            cell_size, cell_size, cell_size, x_size, y_size, z_size, default_value, default_value);
    }
    ...
    {
        Initialize(
            cell_size, cell_size, cell_size, x_size, y_size, z_size, default_value, oob_value);
    }
    ...
    inline void Initialize(const Eigen::Affine3d& origin_transform,
                           const double cell_x_size,
                           const double cell_y_size,
                           const double cell_z_size,
                           const double x_size,
                           ...
                           const T& oob_value)
    {
        ...
        const Eigen::Affine3d origin_transform =
    ...
    inline bool
    IndexInBounds(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
    ...
    GetImmutable(const double x, const double y, const double z) const
    {
    ...
    inline std::pair<const T&, bool>
    GetImmutable(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
        assert(initialized_);
    ...
    GetMutable(const int64_t x_index, const int64_t y_index, const int64_t z_index)
    {
        assert(initialized_);
        if (IndexInBounds(x_index, y_index, z_index)) {
            const int64_t data_index = GetDataIndex(x_index, y_index, z_index);
    ...
    inline bool
    SetValue(const int64_t x_index, const int64_t y_index, const int64_t z_index, const T& value)
    {
        assert(initialized_);
        if (IndexInBounds(x_index, y_index, z_index)) {
            const int64_t data_index = GetDataIndex(x_index, y_index, z_index);
    ...
    inline bool
    SetValue(const int64_t x_index, const int64_t y_index, const int64_t z_index, T&& value)
    {
        assert(initialized_);
        if (IndexInBounds(x_index, y_index, z_index)) {
            const int64_t data_index = GetDataIndex(x_index, y_index, z_index);
    ...
    LocationToGridIndex(const double x, const double y, const double z) const
    {
    ...
    GridIndexToLocation(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
        assert(initialized_);
        if (IndexInBounds(x_index, y_index, z_index)) {
            const Eigen::Vector3d point_in_grid_frame(cell_x_size_ * ((double) x_index + 0.5),
    ...
    inline bool SetRawData(std::vector<T>& data)
    {
        ...
        if ((int64_t) data.size() == expected_length) {
            ...
            std::cerr << "Failed to load internal data - expected " << expected_length << " got "
                      << data.size() << std::endl;
            return false;
        }
    }
    ...
    inline uint64_t
    HashDataIndex(const int64_t x_index, const int64_t y_index, const int64_t z_index) const
    {
        return (x_index * stride1_) + (y_index * stride2_) + z_index;
    }
};
