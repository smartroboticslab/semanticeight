...
namespace se {
  ...
  Objects filter_visible_objects(const Objects&       objects,
  ...
  std::vector<float> object_lod_gain_blocks(const Objects&         objects,
    ...
    for (size_t i = 0; i < objects.size(); i++) {
      ...
      if (visible_objects.count(o.instance_id) == 1) {
        ...
        gains[i] = (num_blocks - o.num_blocks_per_min_scale[0]) / static_cast<float>(num_blocks);
      }
    }
    return gains;
  }
  ...
  std::vector<float> object_lod_gain_raycasting(const Objects&         objects,
    ...
    for (const auto object_id : visible_objects) {
      raycasting_masks.push_back(objects[object_id]->bounding_volume_M_.raycastingMask(image_res, T_MC,
            sensor));
    }
#endif
...
#pragma omp simd
      for (int x = 0; x < image_res.x(); ++x) {
        ...
        for (size_t i = 0; i < visible_objects.size(); i++) {
          ...
          // Raycast the object.
          const Eigen::Vector4f hit_O = ObjVoxelImpl::raycast(
          ...
          if (hit_O.w() >= 0.f) {
            ...
            float fg_prob = object.map_->interpAtPoint(hit_O.head<3>(),
                [](const auto& data){ return data.fg; }).first;
            ...
            // happens.
            const auto* block = object.map_->fetch(object.map_->pointToVoxel(hit_O.head<3>()));
            if (block) {
...
#pragma omp simd
      for (int x = 0; x < image_res.x(); ++x) {
        ...
        for (size_t i = 0; i < visible_objects.size(); i++) {
          ...
          const Eigen::Vector3f t_OC = (T_OM * t_MC).head(3);
          // Raycast the object.
          const Eigen::Vector4f hit_O = ObjVoxelImpl::raycast( *object.map_, t_OC, ray_dir_O,
              raycasting_sensor.nearDist(ray_dir_C), raycasting_sensor.farDist(ray_dir_C));
          if (hit_O.w() >= 0.f) {
            float hit_distance = (t_OC - hit_O.head<3>()).norm();
            // Slightly increase the hit distance for the background and "stuff"
            // so that the foreground has priority
            ...
            if (block) {
              ...
              const Eigen::Vector3f block_centre_coord_f = se::get_sample_coord(block->coordinates(),
                  ObjVoxelImpl::VoxelBlockType::size_li, Eigen::Vector3f::Constant(0.5f));
              const Eigen::Vector3f block_centre_O = object.map_->voxelFToPoint(block_centre_coord_f);
              const Eigen::Vector3f block_centre_C = (T_CM * object.T_MO_ * block_centre_O.homogeneous()).head(3);
              // Keep track of the expected scale when this VoxelBlock is observed and its current
              // minimum scale.
              nearest_hit_expected_scale = sensor.targetIntegrationScale(block_centre_C,
                  object.map_->voxelDim(), ObjVoxelImpl::VoxelBlockType::max_scale);
              nearest_hit_min_scale = block->min_scale();
            } else {
