...
namespace ptp {
  ...
  SafeFlightCorridorGenerator::SafeFlightCorridorGenerator(const OccupancyWorld&   ow,
      ...
      min_flight_corridor_radius_ = pp.robot_radius_ + pp.safety_radius_ + pp.min_control_point_radius_;
  ...
  bool SafeFlightCorridorGenerator::setupPlanner(const Eigen::Vector3f& start_point_M,
    ...
    auto motion_validator(std::shared_ptr<MotionValidatorOccupancyDense>(new MotionValidatorOccupancyDense(si_, &pcc_, min_flight_corridor_radius_)));
  ...
  void SafeFlightCorridorGenerator::reduceToControlPointCorridorRadius(Path<kDim>::Ptr path_m) {
    for (std::vector<State<kDim>>::iterator it_i = path_m->states.begin(); it_i != path_m->states.end(); ++it_i) {
    ...
    if (pdef_) {
      ...
      if (p) {
        ...
        OMPL_INFORM("SimpleSetup: Path simplification took %f seconds and "
                    ...
                    simplifyTime, numStates, path.getStateCount());
        return;
      }
    }
  ...
  void SafeFlightCorridorGenerator::simplifyPath(ompl::geometric::PathGeometric& path) {
    ...
    ob::PlannerTerminationCondition ptc =
            ob::timedPlannerTerminationCondition(max_time);
  ...
  void SafeFlightCorridorGenerator::setSpaceBoundaries(
          const Eigen::Vector3f& min_boundary,
          const Eigen::Vector3f& max_boundary, ob::StateSpacePtr space) {
