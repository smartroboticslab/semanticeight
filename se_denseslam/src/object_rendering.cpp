...
#pragma omp simd
    for (int x = 0; x < surface_point_cloud_M.width(); ++x) {
      ...
      const Eigen::Vector3f ray_dir_M = se::math::to_rotation(raycast_T_MC) * ray_dir_C.normalized();
      ...
      for (size_t i = 0; i < visible_objects_vec.size(); i++) {
        ...
        if (surface_intersection_O.w() >= 0.f) {
          ...
          float fg_prob = object.map_->interpAtPoint(surface_intersection_O.head<3>(),
          ...
          if (object_normal_M.x() != INVALID && object_normal_M.norm() > 0.f) {
            ...
            case RenderMode::Color:
              {
                ...
                // inerpolation.
                const Eigen::Matrix4f& T_OM = object.T_OM_;
                const Eigen::Vector3f point_O = (T_OM * object_point_M.homogeneous()).head<3>();
                const float interp_r = object.map_->interpAtPoint(point_O,
                    [](const auto& data) { return data.r; }).first;
                const float interp_g = object.map_->interpAtPoint(point_O,
                    [](const auto& data) { return data.g; }).first;
                const float interp_b = object.map_->interpAtPoint(point_O,
                    [](const auto& data) { return data.b; }).first;
            ...
            case RenderMode::InstanceID:
              if (instance_id == se::instance_bg) {
                ...
                // Colored objects.
                const Eigen::Vector3f rgb = se::internal::color_map[instance_id % se::internal::color_map.size()] / 255.0f;
                col = dir.cwiseProduct(rgb) + ambient_M;
              }
              break;
            case RenderMode::ClassID:
              {
                ...
                if (class_id == se::class_bg) {
                  ...
                  // Colored objects.
                  const Eigen::Vector3f rgb = se::internal::color_map[class_id % se::internal::color_map.size()] / 255.0f;
                  col = dir.cwiseProduct(rgb) + ambient_M;
                }
              }
              break;
            ...
            case RenderMode::ForegroundProb:
              const Eigen::Matrix4f& T_OM = object.T_OM_;
              const Eigen::Vector3f point_O = (T_OM * object_point_M.homogeneous()).head<3>();
              const float interp_fg = object.map_->interpAtPoint(point_O,
                  [](const auto& data) { return data.fg; }).first;
              if (0.0f <= interp_fg && interp_fg <= 1.0f) {
                col = (dir + ambient_M).cwiseProduct(Eigen::Vector3f(interp_fg, 0.0f, 1.0f - interp_fg));
              } else {
                col = Eigen::Vector3f::Zero();
              }
              break;
            ...
            col *=  255.0f;
            output_image_data[pixel_idx] = se::pack_rgba(col.x(), col.y(), col.z(), 0xFF);
          }
          // Found the object
          break;
        }
...
#if SE_BOUNDING_VOLUME > 0
      objects[i]->bounding_volume_M_.overlay(output_image_data, output_image_res,
          T_MC, sensor, opacity);
#else
#endif
    }
