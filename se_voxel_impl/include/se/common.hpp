                      ...
                      Eigen::Vector3f& point_M)
{
    ...
    if (map.containsPoint(ray_pos_M)) {
        ...
        if (peek_data.y > 0) {
            value = map.interpAtPoint(ray_pos_M, select_node_value, select_voxel_value, 0, is_valid)
                        .first;
        }
    }
    ...
    while (!is_valid) {
        ...
        if (map.containsPoint(ray_pos_M)) {
            ...
            if (peek_data.y > 0) {
                value =
                    map.interpAtPoint(ray_pos_M, select_node_value, select_voxel_value, 0, is_valid)
                        .first;
            }
        }
    }
 ...
 *
 * \param[in]     map                Reference to the octree
 * \param[in]     select_value       lambda function selecting the node and voxel value to be interpolated
 * \param[in]     ray_origin_M       Origin of the ray in map frame [m]
 * \param[in]     ray_dir_M          Direction of the ray in map frame [m]
 * \param[in]     step_size          Size of a step per iteration [m]
 * \param[in]     t_max              Maximum travelled distance along the ray [m]
