...
struct camera_parameter {
    ...
    {
        K_ << K(0), 0, K(2), 0, 0, K(1), K(3), 0, 0, 0, 1, 0, 0, 0, 0, 1;
    };

    float focal_length_pix() const
    {
        return K_(0, 0);
    }
    void setPose(Eigen::Matrix4f T_MC)
    {
        T_MC_ = T_MC;
    }
    Eigen::Vector2i imageResolution() const
    {
        return image_res_;
    }
    ...
    {
        return T_MC_.topRightCorner(3, 1);
    }
    Eigen::Matrix3f R_MC() const
    {
        return T_MC_.topLeftCorner(3, 3);
    };
    Eigen::Matrix4f K() const
    {
        return K_;
    }
...
struct sphere_obstacle : obstacle {
    ...
    sphere_obstacle(const camera_parameter& camera_parameter,
                    const Eigen::Vector2f& centre_angle,
    ...
    void operator()(const camera_parameter& camera_parameter)
    {
        ...
        for (int x = 0; x < depth_image_res.x(); x++) {
            for (int y = 0; y < depth_image_res.y(); y++) {
                ray(x, y);
                float dist(SENSOR_LIMIT);
                for (std::vector<obstacle*>::iterator obstacle = obstacles_.begin();
                     obstacle != obstacles_.end();
                     ++obstacle) {
                ...
                float regularisation = std::sqrt(
                    1 + se::math::sq(std::abs(x + 0.5 - depth_image_res.x() / 2) / focal_length_pix)
                        ...
                        for (int j = 0; j < stride; j += half_step) {
                            for (int i = 0; i < stride; i += half_step) {
                                ...
                                if (voxel_data.y == 0) {
                                    ...
                                    voxel_data.y =
                                        fminf(voxel_data.y + parent_data.delta_y, MAX_WEIGHT);
                                    voxel_data.delta_y = parent_data.delta_y;
                                }
              ...
              float* depth_image_data)
{
    ...
    for (int i = 0; i < num_elem; ++i) {
        ...
        const float scaled_pix =
            (camera_parameter.K().inverse()
             * (Eigen::Vector3f(1, 0, 1) - Eigen::Vector3f(0, 0, 1)).homogeneous())
                .x();

        // Calculate the maximum uncertainty possible
        int scale =
            compute_scale((block_coord + Eigen::Vector3i::Constant(block_size / 2)).cast<float>(),
                          t_CM,
                          scaled_pix,
                          voxel_dim);
        if (SCALE != 4)
            ...
            for (float y = stride / 2; y < block_size; y += stride) {
                for (float x = stride / 2; x < block_size; x += stride) {
                    ...
                    const Eigen::Vector3f point_C =
                        (T_CM * (voxel_dim * voxel_coord.cast<float>()).homogeneous()).head(3);
                    ...
                    const Eigen::Vector2f pixel_f = Eigen::Vector2f(
                        pixel_homo.x() * inverse_depth_value, pixel_homo.y() * inverse_depth_value);
                    if (pixel_f.x() < 0.5f || pixel_f.x() > depth_image_res.x() - 1.5f
                        || pixel_f.y() < 0.5f || pixel_f.y() > depth_image_res.y() - 1.5f)
                        continue;

                    float depth_value =
                        depth_image_data[int(pixel_f.x()) + depth_image_res.x() * int(pixel_f.y())];
                    const float dist = (depth_value - point_C.z())
                    ...
                    if (dist > -MU) {
                        ...
                        // Update TSDF value
                        parent_data.x =
                            (parent_data.x * parent_data.y + sample) / (parent_data.y + 1);
...
template<typename T>
std::vector<VoxelBlockType<MultiresTSDF::VoxelType>*>
buildActiveList(se::Octree<T>& map,
                const camera_parameter& camera_parameter,
                float voxel_dim,
                SensorImpl& sensor)
{
...
TEST_F(MultiscaleTSDFMovingCameraTest, SphereTranslation)
{
    ...
    spheres.push_back(new sphere_obstacle(
        voxel_dim_ * Eigen::Vector3f(size_ * 1 / 2, size_ * 1 / 2, size_ / 2), 0.5f));
    ...
    for (int frame = 0; frame < frames; frame++) {
        ...
        T_MC.topRightCorner<3, 1>() =
            (R_MB * Eigen::Vector3f(-(size_ / 2 + frame * size_ / 8), 0, size_ / 2)
             + Eigen::Vector3f(size_ / 2, size_ / 2, 0))
            * voxel_dim_;
        ...
        f << "./out/scale_" + std::to_string(SCALE) + "-sphere-linear_back_move-"
    ...
    for (std::vector<obstacle*>::iterator sphere = spheres.begin(); sphere != spheres.end();
         ++sphere) {
        delete *sphere;
    }
...
TEST_F(MultiscaleTSDFMovingCameraTest, SphereRotation)
{
    ...
    // Allocate spheres in world frame
    sphere_obstacle* sphere_close = new sphere_obstacle(
        voxel_dim_ * Eigen::Vector3f(size_ * 1 / 8, size_ * 2 / 3, size_ / 2), 0.3f);
    sphere_obstacle* sphere_far = new sphere_obstacle(
        voxel_dim_ * Eigen::Vector3f(size_ * 7 / 8, size_ * 1 / 3, size_ / 2), 0.3f);
    ...
    for (int frame = 0; frame < frames; frame++) {
        ...
        Eigen::Matrix3f R_MB;
        R_MB << std::cos(angle), -std::sin(angle), 0, std::sin(angle), std::cos(angle), 0, 0, 0, 1;

        T_MC.topLeftCorner<3, 3>() = R_MB * R_BC;

        T_MC.topRightCorner<3, 1>() =
            (R_MB * Eigen::Vector3f(-(size_ / 2 + 16 * size_ / 8), 0, size_ / 2)
             + Eigen::Vector3f(size_ / 2, size_ / 2, 0))
            * voxel_dim_;
        ...
        f << "./out/scale_" + std::to_string(SCALE) + "-sphere-rotational_move-"
                + std::to_string(frame) + ".vtk";
...
TEST_F(MultiscaleTSDFMovingCameraTest, BoxTranslation)
{
    ...
    boxes.push_back(
        new box_obstacle(voxel_dim_ * Eigen::Vector3f(size_ * 1 / 2, size_ * 1 / 4, size_ / 2),
                         voxel_dim_ * Eigen::Vector3f(size_ * 1 / 4, size_ * 1 / 4, size_ / 4)));
    boxes.push_back(
        new box_obstacle(voxel_dim_ * Eigen::Vector3f(size_ * 1 / 2, size_ * 3 / 4, size_ / 2),
                         voxel_dim_ * Eigen::Vector3f(size_ * 1 / 4, size_ * 1 / 4, size_ / 4)));
    ...
    for (int frame = 0; frame < frames; frame++) {
        ...
        f << "./out/scale_" + std::to_string(SCALE) + "-box-linear_back_move-"
                + std::to_string(frame) + ".vtk";
...
TEST_F(MultiscaleTSDFMovingCameraTest, SphereBoxTranslation)
{
    ...
    obstacles.push_back(
        new box_obstacle(voxel_dim_ * Eigen::Vector3f(size_ * 1 / 2, size_ * 1 / 4, size_ / 2),
                         voxel_dim_ * Eigen::Vector3f(size_ * 1 / 4, size_ * 1 / 4, size_ / 4)));
    obstacles.push_back(new sphere_obstacle(
        voxel_dim_ * Eigen::Vector3f(size_ * 1 / 2, size_ * 1 / 2, size_ / 2), 0.5f));
    ...
    for (int frame = 0; frame < frames; frame++) {
        ...
        f << "./out/scale_" + std::to_string(SCALE) + "-sphere-and-box-linear_back_move-"
                + std::to_string(frame) + ".vtk";
    ...
    for (std::vector<obstacle*>::iterator obstacle = obstacles.begin(); obstacle != obstacles.end();
         ++obstacle) {
        delete *obstacle;
    }
    free(depth_image_data_);
