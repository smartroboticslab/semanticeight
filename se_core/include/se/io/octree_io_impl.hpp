                                ...
                                const int min_scale)
{
    auto get_value = [&](int x, int y, int z) {
        ...
        if (scale >= VoxelT::VoxelBlockType::
                max_scale) { // scale == max_scale should be the same value in the node and voxel
...
template<typename T>
int se::save_octree_structure_ply(const se::Octree<T>& octree, const std::string filename)
{
    // Open the file for writing.
    std::ofstream file(filename.c_str());
    if (!file.is_open()) {
    ...
    while (node) {
        ...
        for (int i = 0; i < 8; ++i) {
            ss_nodes_corners << node_corners[i].x() << " " << node_corners[i].y() << " "
