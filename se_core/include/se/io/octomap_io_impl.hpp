...
octomap::OcTree* se::to_octomap(const se::Octree<VoxelT>& octree, const FunctionT set_node_value)
{
    ...
    if (node != nullptr) {
        ...
        while (stack_idx != 0) {
            ...
            if (node->isBlock()) {
                const VoxelBlockType<VoxelT>* block = static_cast<VoxelBlockType<VoxelT>*>(node);
                ...
                for (int z = block_coord.z(); z < z_last; ++z) {
                    for (int y = block_coord.y(); y < y_last; ++y) {
                        for (int x = block_coord.x(); x < x_last; ++x) {
                            ...
                            const octomap::point3d voxel_coord(voxel_coord_eigen.x(),
                                                               voxel_coord_eigen.y(),
                                                               voxel_coord_eigen.z());
