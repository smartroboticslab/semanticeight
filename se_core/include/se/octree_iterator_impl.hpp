...
bool se::OctreeIterator<T>::operator==(const se::OctreeIterator<T>& other) const
{
    ...
    return (node_stack_ == other.node_stack_) && (parent_stack_ == other.parent_stack_)
        && (child_idx_stack_ == other.child_idx_stack_) && (voxel_idx_ == other.voxel_idx_)
        && (voxel_scale_ == other.voxel_scale_) && (voxel_dim_ == other.voxel_dim_);
}
    ...
    while (true) {
        ...
        if ((node != nullptr) && node->isBlock()) {
            ...
            if (T::isValid(data)) {
                ...
                const Eigen::Vector3i volume_coord =
                    block->voxelCoordinates(voxel_idx_, voxel_scale_);
                const Eigen::Vector3f centre_M = voxel_dim_
                    * (volume_coord.cast<float>() + Eigen::Vector3f::Constant(size / 2.0f));
