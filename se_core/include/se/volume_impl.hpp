...
template<typename T>
se::Volume<T>::Volume() :
        centre_M(Eigen::Vector3f::Constant(-1.0f)), dim(0.0f), size(0), data(T::initData())
{
}
...
se::Volume<T>::Volume(const Eigen::Vector3f& _centre_M,
        ...
        centre_M(_centre_M), dim(_dim), size(_size), data(_data)
{
}
...
se::Volume<T>::Volume(const se::Volume<T>& other) :
        centre_M(other.centre_M), dim(other.dim), size(other.size), data(other.data)
{
}
...
{
    return (centre_M == other.centre_M) && (dim == other.dim) && (size == other.size)
        && (data == other.data);
}
