...
namespace dbg {
  ...
  class DebugImages {
    public:
      ...
      void init(const size_t num_images,
                const size_t width,
                const size_t height) {
      ...
      void save(const std::string& directory,
        ...
        const int status = mkdir(directory.c_str(),
            S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
        if ((status != 0) && (errno != EEXIST)) {
          std::cerr << "Warning: debug image output directory "
        ...
        for (size_t i = 0; i < images_.size(); ++i) {
          ...
          const std::string filename = directory + "/"
              + basename + "_" + std::to_string(i) + ".png";
          // Save the image as PNG.
          const unsigned status = lodepng_encode32_file(filename.c_str(),
              reinterpret_cast<const unsigned char*>(images_[i].data()),
              images_[i].width(), images_[i].height());
          if (status != 0) {
            std::cerr << "Warning: debug image "
              << filename << " could not be created.\n";
          }
        }
      }

      void set(const size_t   image_index,
               const size_t   x,
               const size_t   y,
