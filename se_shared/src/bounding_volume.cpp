 ...
 * x y].
 */
inline Eigen::Matrix<float, 1, 5> point_to_conic_powers(
...
bool se::BoundingSphere::isVisible(const Eigen::Matrix4f&   T_VC,
  ...
  const Eigen::Vector3f center_c
      = (T_VC.inverse() * center_.homogeneous()).head<3>();
...
void se::BoundingSphere::computeProjection(const se::PinholeCamera& camera,
  ...
  if (dist_to_sphere_center >= radius_) {
    ...
    const Eigen::Vector3f ray_plane_intersection (0.f, 0.f,
        neg_d / dir_c.z());
    ...
    // 0 degrees.
    const Eigen::Vector3f p0 = (T_CV
        * Eigen::Vector4f(visible_circle_radius, 0.f, 0.f, 1.f)).head<3>();
    // 90 degrees.
    const Eigen::Vector3f p1 = (T_CV
        * Eigen::Vector4f(0.f, visible_circle_radius, 0.f, 1.f)).head<3>();
    // 180 degrees.
    const Eigen::Vector3f p2 = (T_CV
        * Eigen::Vector4f(-visible_circle_radius, 0.f, 0.f, 1.f)).head<3>();
    // 270 degrees.
    const Eigen::Vector3f p3 = (T_CV
        * Eigen::Vector4f(0.f, -visible_circle_radius, 0.f, 1.f)).head<3>();
    // 45 degrees.
    const Eigen::Vector3f p4 = (T_CV
        * Eigen::Vector4f(0.707106781186548f * visible_circle_radius,
        0.707106781186548f * visible_circle_radius, 0.f, 1.f)).head<3>();
    ...
    const Eigen::Matrix<float, 5, 1> h
        = Eigen::Matrix<float, 5, 1>::Constant(-F);
    // Solve the linear system.
    const Eigen::Matrix<float, 5, 1> ellipse_coefficients
        = H.colPivHouseholderQr().solve(h);
    ...
    // Ellipse axes.
    a_ = -sqrt(2 * (A * E * E + C * D * D - B * D * E + F * tmp1)
        * (A + C + tmp2)) / tmp1;
    b_ = -sqrt(2 * (A * E * E + C * D * D - B * D * E + F * tmp1)
        * (A + C - tmp2)) / tmp1;
    // Ellipse center.
    x_c_ = (2 * C * D - B * E) / tmp1;
    y_c_ = (2 * A * E - B * D) / tmp1;
    ...
    const float delta = (A * C - B * B / 4.f) * F
        + B * E * D / 4.f - C * D * D / 4.f - A * E * E / 4.f;
...
cv::Mat se::BoundingSphere::raycastingMask(const Eigen::Vector2i&   mask_size,
  ...
  if (is_ellipse_ == 1) {
    ...
    cv::ellipse(raycasting_mask, cv::Point(x_c_, y_c_), cv::Size(a_, b_), theta_, 0, 360, 255, -1);
...
void se::BoundingSphere::overlay(uint32_t*                out,
  ...
  cv::circle(out_mat, cv::Point(center_px.x(), center_px.y()),
      point_radius_px, _overlay_color, -1);

  if (is_ellipse_ == 1) {
    // Draw the ellipse outline.
    cv::ellipse(out_mat, cv::Point(x_c_, y_c_),
        cv::Size(a_, b_), theta_, 0, 360,
        _overlay_color);
  }
}
...
se::AABB::AABB(const Eigen::Vector3f& min,
...
bool se::AABB::isVisible(const Eigen::Matrix4f&   T_VC,
                         const se::PinholeCamera& camera) const {
  const Eigen::Matrix4f T_CV = T_VC.inverse();
  ...
  for (int i = 0; i < vertices_.cols(); ++i) {
    const Eigen::Vector3f vertex_c
        = (T_CV * vertices_.col(i).homogeneous()).head<3>();
...
std::vector<cv::Point2f> se::AABB::projectAABB(
    const Eigen::Matrix4f&   T_VC,
    const se::PinholeCamera& camera) const {
