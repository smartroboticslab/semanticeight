...
bool read_masks(const std::string& filename, std::vector<cv::Mat>& masks) {
  ...
  if (num_masks == 0) {
    ...
    if (masks_npy.word_size != sizeof(numpy_mask_t)) {
        std::cerr << "Error reading masks: Numpy word size must be "
            << sizeof(numpy_mask_t)
    ...
    // Read masks
    const numpy_mask_t* current_mask_npy
        = masks_npy.data<numpy_mask_t>();
    for (size_t i=0; i<num_masks; ++i) {
      // Initialize mask
      masks[i] = cv::Mat(cv::Size(width, height), se::mask_t);
      // Copy mask data
      std::memcpy(masks[i].data, &(current_mask_npy[i * width * height]),
  ...
  if (num_objects == 0) {
    ...
    if (class_ids_npy.word_size != sizeof(numpy_class_id_t)) {
        std::cerr << "Error reading class IDs: Numpy word size must be "
            << sizeof(numpy_class_id_t)
            << ", not " << class_ids_npy.word_size << std::endl;
        return false;
    }
  ...
  if (num_objects == 0) {
    ...
    if (all_probs_npy.word_size != sizeof(numpy_confidence_t)) {
        std::cerr << "Error reading all class confidence: Numpy word size must be "
            << sizeof(numpy_confidence_t)
            << ", not " << all_probs_npy.word_size << std::endl;
        return false;
    }
...
namespace se {
  // SegmentationResult ///////////////////////////////////////////////////////
  SegmentationResult::SegmentationResult(const int w, const int h)
    : width(w), height(h) {}
  ...
  bool SegmentationResult::read(const std::string& base_dir,
    ...
    // Read masks
    std::string filename_masks = base_dir + "/"
        + segmentation_masks_dir + "/" + base_name + ".npy";
    std::vector<cv::Mat> masks;
    success = read_masks(filename_masks, masks);
    if (not success) {
      std::cerr << "Could not read segmentation masks from " << filename_masks
          << std::endl;
      return false;
    }
    ...
    // Read class probabilities
    std::string filename_all_probs = base_dir + "/"
        + segmentation_confidence_all_dir + "/" + base_name + ".npy";
    se::VecDetectionConfidence all_probs;
    success = read_all_confs(filename_all_probs, all_probs);
    if (not success) {
      std::cerr << "Could not read class confidence from " << filename_all_probs
          << std::endl;
      return false;
    }
    ...
    for (size_t i=0; i<num_objects; ++i) {
      object_instances.push_back(
          InstanceSegmentation(se::instance_new, masks[i], all_probs[i]));
    }
  ...
  InstanceSegmentationVecIt SegmentationResult::find(const int instance_id) {
    for (InstanceSegmentationVecIt it = object_instances.begin();
        it != object_instances.end(); ++it) {
  ...
  InstanceSegmentationVecCIt SegmentationResult::find(const int instance_id) const {
    for (InstanceSegmentationVecCIt it = object_instances.cbegin();
        it != object_instances.cend(); ++it) {
      if (it->instance_id == instance_id)
        return it;
    }
    // No match found.
  ...
  void SegmentationResult::generateBackgroundInstance() {
    ...
    object_instances.push_back(
        InstanceSegmentation(se::instance_new, se::class_bg, bg_mask));
  }
  ...
  void SegmentationResult::removeSmall(const float percent_nonzero_threshold) {
    ...
    for (int i = object_instances.size()-1; i >= 0; --i) {
      ...
      const size_t total_elements = object_instances[i].instance_mask.rows
          * object_instances[i].instance_mask.cols;
  ...
  void SegmentationResult::removeInvalidDepth(const cv::Mat& valid_depth_mask) {
    ...
    for (int i = object_instances.size()-1; i >= 0; --i) {
      ...
      cv::bitwise_and(object_instances[i].instance_mask, valid_depth_mask,
          mask_intersection);
  ...
  int SegmentationResult::merge(const InstanceSegmentation& other,
  ...
  int SegmentationResult::merge(const SegmentationResult& other,
                                const float               overlap_thres) {
    int num_merged = 0;
