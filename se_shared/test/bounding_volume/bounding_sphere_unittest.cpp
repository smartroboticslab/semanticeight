 ...
 protected:
  ...
  void SetUp() override {
    ...
    readTestResults(test_data_directory + "/camera_hd.txt", camera_hd,
        centers_c_hd, radii_hd, a_hd, b_hd, x_c_hd, y_c_hd, theta_hd,
        valid_ellipse_hd);
    ...
    readTestResults(test_data_directory + "/camera_ros.txt", camera_ros,
        centers_c_ros, radii_ros, a_ros, b_ros, x_c_ros, y_c_ros, theta_ros,
        valid_ellipse_ros);
  ...
  const se::SensorConfig default_config = {640, 480, false, 0.1f, 4.0f, 525.0f, 525.0f, 319.5f, 239.5f};
...
TEST_F(BoundingSphereTest, computeProjection) {
  // HD camera.
  for (size_t i = 0; i < bounding_spheres_hd.size(); ++i) {
    bounding_spheres_hd[i].computeProjection(camera_hd,
